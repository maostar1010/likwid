...
#define HELP_MSG                                                                                   \
    printf("Threaded Memory Hierarchy Benchmark -- Version %d.%d.%d \n\n",                         \
        VERSION,                                                                                   \
        RELEASE,                                                                                   \
        MINORVERSION);                                                                             \
    printf("\n");                                                                                  \
    ...
    printf("-w/--workgroup\t\t <thread_domain>:<size>[:<num_threads>[:<chunk "                     \
           "size>:<stride>]-<streamId>:<domain_id>[:<offset>]\n");                                 \
    ...
    printf("-w allocates the streams in the thread_domain with one thread and support placement "  \
           "of streams\n");                                                                        \
    ...
    printf("# Run the copy benchmark on one CPU at CPU socket 0 with a vector size of 100MB but "  \
           "place one stream on CPU socket 1\n");                                                  \
...
void illhandler(int signum, siginfo_t *info, void *ptr)
{
    ...
    fprintf(stderr,
        "This happens if you want to run a kernel that uses instructions not available on your "
        "system.\n");
    exit(EXIT_FAILURE);
}
...
int main(int argc, char **argv)
{
        ...
        case 'W':
            ...
            if (unknown == 0) {
                ...
                for (size_t i = 0; i < test->streams; i++) {
                    if (currentWorkgroup->streams[i].offset % test->stride) {
                        fprintf(
                            stderr, "Error: Stream %zu: offset is not a multiple of stride!\n", i);
                        return EXIT_FAILURE;
                    }
                    if ((int)(floor(orig_size / currentWorkgroup->numberOfThreads)) %
                        test->stride) {
                        int typesize = allocator_dataTypeLength(test->type);
                        newsize = (((size_t)(floor(orig_size / nrThreads)) / stride) * (stride)) *
                                  nrThreads;
                        if (newsize > 0 && warn_once) {
                            fprintf(stdout,
                                "Warning: Sanitizing vector length to a multiple of the loop "
                                "stride %zu and thread count %u from %ld elements (%ld bytes) to "
                                "%ld elements (%ld bytes)\n",
                                stride,
                                nrThreads,
                                orig_size,
                                orig_size * typesize,
                            ...
                            fprintf(stderr,
                                "Error: The given vector length of %dB is too small to fit %u "
                                "threads because each loop iteration of kernel '%s' requires %zu "
                                "Bytes (%d x %zuB = %zuB). So the minimal selectable size for the "
                                "kernel is %zuB.\n",
                                given,
                                nrThreads,
                                test->name,
            ...
            if (nrThreads > 1) {
                if (currentWorkgroup->init_per_thread) {
                    fprintf(stdout,
                        "Initialization: Each thread in domain initializes its own stream "
                        "chunks\n");
                } else {
                    fprintf(stdout,
    ...
    if (numberOfWorkgroups > 1) {
        ...
        for (uint64_t i = 1; i < numberOfWorkgroups; i++) {
            if (g0_numberOfThreads != groups[i].numberOfThreads) {
                fprintf(stdout,
                    "Warning: Multiple workgroups with different thread counts are not "
                    "recommended. Use with case!\n");
                break;
            }
        }
        for (uint64_t i = 1; i < numberOfWorkgroups; i++) {
            if (g0_size != groups[i].size) {
                fprintf(stdout,
                    "Warning: Multiple workgroups with different sizes are not recommended. Use "
                    "with case!\n");
                break;
            }
        }
    ...
    if ((test->instr_loop > 0) && (test->instr_const > 0)) {
        ownprintf("Instructions:\t\t%" PRIu64 "\n",
            LLU_CAST((double)realSize / test->stride) * test->instr_loop *
