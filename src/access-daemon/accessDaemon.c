...
static void clientmem_finalize()
{
    if (isClientMem) {
...
static void servermem_finalize()
{
    if (isServerMem) {
        int i = 0;
        int j = 0;
...
void __attribute__((constructor(101))) init_accessdaemon(void)
{
    ...
    for (int i = 0; i < avail_sockets; i++) {
        for (int j = 0; j < avail_cpus; j++) {
            int ret = snprintf(
                fname, 1023, "/sys/devices/system/cpu/cpu%d/topology/physical_package_id", j);
            if (ret < 0) {
                continue;
            }
            fname[ret] = '\0';
...
{
    if (((reg & 0x0F0U) == 0x0C0U) || ((reg & 0x190U) == 0x180U) ||
        ((reg & 0x190U) == 0x190U && num_pmc_counters > 4) || ((reg & 0xF00U) == 0x300U) ||
        ((reg & 0xF00U) == 0xC00U) || ((reg & 0xF00U) == 0xD00U) || ((reg & 0xF00U) == 0xE00U) ||
        ((reg & 0xF00U) == 0xF00U) || (reg == 0x48) || (reg == 0x1A0) || (reg == 0x1A4) ||
        (reg == 0x0CE) || (reg == 0x19C) || (reg == 0x1A2) || (reg == 0x1AD) || (reg == 0x1AE) ||
        (reg == 0x1AF) || (reg == 0x1AC) || (reg == 0x1A6) || (reg == 0x1A7) || (reg == 0x620) ||
        (reg == 0xCD) || (reg == 0x1B0) || (reg == 0x1B1)) {
        return 1;
    } else {
        return 0;
    }
...
{
    if ((allowed_intel(reg)) || ((reg & 0xF00U) == 0x600U) || ((reg & 0xF00U) == 0x700U) ||
        (reg == MSR_MPERF) || (reg == MSR_APERF) || (reg == MSR_PERF_STATUS) ||
        (reg == MSR_ALT_PEBS)) {
        return 1;
    }
    return 0;
...
static int allowed_pci_sandybridge(PciDeviceType type, uint32_t reg)
{
    ...
    case NODEVTYPE:
        ...
        break;
    case R3QPI:
        if ((reg == PCI_UNC_R3QPI_PMON_BOX_CTL) || (reg == PCI_UNC_R3QPI_PMON_BOX_STATUS) ||
            (reg == PCI_UNC_R3QPI_PMON_CTL_0) || (reg == PCI_UNC_R3QPI_PMON_CTL_1) ||
            (reg == PCI_UNC_R3QPI_PMON_CTL_2) || (reg == PCI_UNC_R3QPI_PMON_CTR_0_A) ||
            (reg == PCI_UNC_R3QPI_PMON_CTR_1_A) || (reg == PCI_UNC_R3QPI_PMON_CTR_2_A) ||
            (reg == PCI_UNC_R3QPI_PMON_CTR_0_B) || (reg == PCI_UNC_R3QPI_PMON_CTR_1_B) ||
        ...
        break;
    case R2PCIE:
        if ((reg == PCI_UNC_R2PCIE_PMON_BOX_CTL) || (reg == PCI_UNC_R2PCIE_PMON_BOX_STATUS) ||
            (reg == PCI_UNC_R2PCIE_PMON_CTL_0) || (reg == PCI_UNC_R2PCIE_PMON_CTL_1) ||
            (reg == PCI_UNC_R2PCIE_PMON_CTL_2) || (reg == PCI_UNC_R2PCIE_PMON_CTL_3) ||
            (reg == PCI_UNC_R2PCIE_PMON_CTR_0_A) || (reg == PCI_UNC_R2PCIE_PMON_CTR_1_A) ||
            (reg == PCI_UNC_R2PCIE_PMON_CTR_2_A) || (reg == PCI_UNC_R2PCIE_PMON_CTR_3_A) ||
            (reg == PCI_UNC_R2PCIE_PMON_CTR_0_B) || (reg == PCI_UNC_R2PCIE_PMON_CTR_1_B) ||
            (reg == PCI_UNC_R2PCIE_PMON_CTR_2_B) || (reg == PCI_UNC_R2PCIE_PMON_CTR_3_B)) {
            return 1;
        }
        return 0;
        break;
    case IMC:
        if ((reg == PCI_UNC_MC_PMON_BOX_CTL) || (reg == PCI_UNC_MC_PMON_BOX_STATUS) ||
            (reg == PCI_UNC_MC_PMON_CTL_0) || (reg == PCI_UNC_MC_PMON_CTL_1) ||
            (reg == PCI_UNC_MC_PMON_CTL_2) || (reg == PCI_UNC_MC_PMON_CTL_3) ||
            (reg == PCI_UNC_MC_PMON_CTR_0_A) || (reg == PCI_UNC_MC_PMON_CTR_1_A) ||
            (reg == PCI_UNC_MC_PMON_CTR_2_A) || (reg == PCI_UNC_MC_PMON_CTR_3_A) ||
            (reg == PCI_UNC_MC_PMON_CTR_0_B) || (reg == PCI_UNC_MC_PMON_CTR_1_B) ||
            (reg == PCI_UNC_MC_PMON_CTR_2_B) || (reg == PCI_UNC_MC_PMON_CTR_3_B) ||
            (reg == PCI_UNC_MC_PMON_FIXED_CTL) || (reg == PCI_UNC_MC_PMON_FIXED_CTR_A) ||
        ...
        break;
    case HA:
        if ((reg == PCI_UNC_HA_PMON_BOX_CTL) || (reg == PCI_UNC_HA_PMON_BOX_STATUS) ||
            (reg == PCI_UNC_HA_PMON_CTL_0) || (reg == PCI_UNC_HA_PMON_CTL_1) ||
            (reg == PCI_UNC_HA_PMON_CTL_2) || (reg == PCI_UNC_HA_PMON_CTL_3) ||
            (reg == PCI_UNC_HA_PMON_CTR_0_A) || (reg == PCI_UNC_HA_PMON_CTR_1_A) ||
            (reg == PCI_UNC_HA_PMON_CTR_2_A) || (reg == PCI_UNC_HA_PMON_CTR_3_A) ||
            (reg == PCI_UNC_HA_PMON_CTR_0_B) || (reg == PCI_UNC_HA_PMON_CTR_1_B) ||
            (reg == PCI_UNC_HA_PMON_CTR_2_B) || (reg == PCI_UNC_HA_PMON_CTR_3_B) ||
            (reg == PCI_UNC_HA_PMON_OPCODEMATCH) || (reg == PCI_UNC_HA_PMON_ADDRMATCH0) ||
        ...
        break;
    case QPI:
        if ((reg == PCI_UNC_QPI_PMON_BOX_CTL) || (reg == PCI_UNC_QPI_PMON_BOX_STATUS) ||
            (reg == PCI_UNC_QPI_PMON_CTL_0) || (reg == PCI_UNC_QPI_PMON_CTL_1) ||
            (reg == PCI_UNC_QPI_PMON_CTL_2) || (reg == PCI_UNC_QPI_PMON_CTL_3) ||
            (reg == PCI_UNC_QPI_PMON_CTR_0_A) || (reg == PCI_UNC_QPI_PMON_CTR_1_A) ||
            (reg == PCI_UNC_QPI_PMON_CTR_2_A) || (reg == PCI_UNC_QPI_PMON_CTR_3_A) ||
            (reg == PCI_UNC_QPI_PMON_CTR_0_B) || (reg == PCI_UNC_QPI_PMON_CTR_1_B) ||
            (reg == PCI_UNC_QPI_PMON_CTR_2_B) || (reg == PCI_UNC_QPI_PMON_CTR_3_B) ||
            (reg == PCI_UNC_QPI_PMON_MASK_0) || (reg == PCI_UNC_QPI_PMON_MASK_1) ||
            (reg == PCI_UNC_QPI_PMON_MATCH_0) || (reg == PCI_UNC_QPI_PMON_MATCH_1) ||
        ...
        break;
    case IRP:
        if ((reg == PCI_UNC_IRP_PMON_BOX_STATUS) || (reg == PCI_UNC_IRP_PMON_BOX_CTL) ||
            (reg == PCI_UNC_IRP0_PMON_CTL_0) || (reg == PCI_UNC_IRP0_PMON_CTL_1) ||
            (reg == PCI_UNC_IRP0_PMON_CTR_0) || (reg == PCI_UNC_IRP0_PMON_CTR_1) ||
            (reg == PCI_UNC_IRP1_PMON_CTL_0) || (reg == PCI_UNC_IRP1_PMON_CTL_1) ||
            (reg == PCI_UNC_IRP1_PMON_CTR_0) || (reg == PCI_UNC_IRP1_PMON_CTR_1)) {
            return 1;
        }
        return 0;
        break;
...
{
    if ((allowed_intel(reg)) || (allowed_sandybridge(reg)) || ((reg & 0xF00U) == 0x700U)) {
        return 1;
    } else {
        return 0;
    }
...
static int allowed_pci_haswell(PciDeviceType type, uint32_t reg)
{
    ...
    case HA:
        ...
        break;
    case QPI:
        if ((reg == PCI_UNC_V3_QPI_PMON_BOX_CTL) || (reg == PCI_UNC_V3_QPI_PMON_BOX_STATUS) ||
            (reg == PCI_UNC_V3_QPI_PMON_CTL_0) || (reg == PCI_UNC_V3_QPI_PMON_CTL_1) ||
            (reg == PCI_UNC_V3_QPI_PMON_CTL_2) || (reg == PCI_UNC_V3_QPI_PMON_CTL_3) ||
            (reg == PCI_UNC_V3_QPI_PMON_CTR_0_A) || (reg == PCI_UNC_V3_QPI_PMON_CTR_1_A) ||
            (reg == PCI_UNC_V3_QPI_PMON_CTR_2_A) || (reg == PCI_UNC_V3_QPI_PMON_CTR_3_A) ||
            (reg == PCI_UNC_V3_QPI_PMON_CTR_0_B) || (reg == PCI_UNC_V3_QPI_PMON_CTR_1_B) ||
            (reg == PCI_UNC_V3_QPI_PMON_CTR_2_B) || (reg == PCI_UNC_V3_QPI_PMON_CTR_3_B) ||
            (reg == PCI_UNC_V3_QPI_PMON_RX_MASK_0) || (reg == PCI_UNC_V3_QPI_PMON_RX_MASK_1) ||
            (reg == PCI_UNC_V3_QPI_PMON_RX_MATCH_0) || (reg == PCI_UNC_V3_QPI_PMON_RX_MATCH_1) ||
            (reg == PCI_UNC_V3_QPI_PMON_TX_MASK_0) || (reg == PCI_UNC_V3_QPI_PMON_TX_MASK_1) ||
            (reg == PCI_UNC_V3_QPI_PMON_TX_MATCH_0) || (reg == PCI_UNC_V3_QPI_PMON_TX_MATCH_1) ||
            (reg == PCI_UNC_V3_QPI_RATE_STATUS) || (reg == PCI_UNC_V3_QPI_LINK_LLR) ||
...
static int allowed_silvermont(uint32_t reg)
{
    ...
    if (((reg & 0x0F8U) == 0x0C0U) || ((reg & 0xFF0U) == 0x180U) || ((reg & 0xF00U) == 0x300U) ||
        ((reg & 0xF00U) == 0x600U) || ((reg & 0xF00U) == 0xC00U) || ((reg & 0xF00U) == 0xD00U) ||
        (reg == 0x1A0) || (reg == 0x0CE) || (reg == 0x1AD) || (reg == 0x19C) || (reg == 0x1A2) ||
        (reg == 0x1A6) || (reg == 0x1A7)) {
        return 1;
    } else {
        return 0;
...
static int allowed_knl(uint32_t reg)
{
    if (allowed_silvermont(reg))
        ...
        if (((reg & 0xF00U) == 0x700U) || ((reg & 0xF00U) == 0xE00U) ||
            ((reg & 0xF00U) == 0xF00U) || (reg == MSR_PREFETCH_ENABLE))
            return 1;
    }
    return 0;
}
...
static int allowed_pci_knl(PciDeviceType type, uint32_t reg)
{
    switch (type) {
    case EDC:
        if ((reg == PCI_MIC2_EDC_U_CTR0_A) || (reg == PCI_MIC2_EDC_U_CTR0_B) ||
            (reg == PCI_MIC2_EDC_U_CTR1_A) || (reg == PCI_MIC2_EDC_U_CTR1_B) ||
            (reg == PCI_MIC2_EDC_U_CTR2_A) || (reg == PCI_MIC2_EDC_U_CTR2_B) ||
            (reg == PCI_MIC2_EDC_U_CTR3_A) || (reg == PCI_MIC2_EDC_U_CTR3_B) ||
            (reg == PCI_MIC2_EDC_U_CTRL0) || (reg == PCI_MIC2_EDC_U_CTRL1) ||
            (reg == PCI_MIC2_EDC_U_CTRL2) || (reg == PCI_MIC2_EDC_U_CTRL3) ||
            (reg == PCI_MIC2_EDC_U_BOX_CTRL) || (reg == PCI_MIC2_EDC_U_BOX_STATUS) ||
            (reg == PCI_MIC2_EDC_U_FIXED_CTR_A) || (reg == PCI_MIC2_EDC_U_FIXED_CTR_B) ||
            (reg == PCI_MIC2_EDC_U_FIXED_CTRL) || (reg == PCI_MIC2_EDC_D_CTR0_A) ||
            (reg == PCI_MIC2_EDC_D_CTR0_B) || (reg == PCI_MIC2_EDC_D_CTR1_A) ||
            (reg == PCI_MIC2_EDC_D_CTR1_B) || (reg == PCI_MIC2_EDC_D_CTR2_A) ||
            (reg == PCI_MIC2_EDC_D_CTR2_B) || (reg == PCI_MIC2_EDC_D_CTR3_A) ||
            (reg == PCI_MIC2_EDC_D_CTR3_B) || (reg == PCI_MIC2_EDC_D_CTRL0) ||
            (reg == PCI_MIC2_EDC_D_CTRL1) || (reg == PCI_MIC2_EDC_D_CTRL2) ||
            (reg == PCI_MIC2_EDC_D_CTRL3) || (reg == PCI_MIC2_EDC_D_BOX_CTRL) ||
            (reg == PCI_MIC2_EDC_D_BOX_STATUS) || (reg == PCI_MIC2_EDC_D_FIXED_CTR_A) ||
            (reg == PCI_MIC2_EDC_D_FIXED_CTR_B) || (reg == PCI_MIC2_EDC_D_FIXED_CTRL)) {
            return 1;
        }
        break;
    case IMC:
        if ((reg == PCI_MIC2_MC_U_CTR0_A) || (reg == PCI_MIC2_MC_U_CTR0_B) ||
            (reg == PCI_MIC2_MC_U_CTR1_A) || (reg == PCI_MIC2_MC_U_CTR1_B) ||
            (reg == PCI_MIC2_MC_U_CTR2_A) || (reg == PCI_MIC2_MC_U_CTR2_B) ||
            (reg == PCI_MIC2_MC_U_CTR3_A) || (reg == PCI_MIC2_MC_U_CTR3_B) ||
            (reg == PCI_MIC2_MC_U_CTRL0) || (reg == PCI_MIC2_MC_U_CTRL1) ||
            (reg == PCI_MIC2_MC_U_CTRL2) || (reg == PCI_MIC2_MC_U_CTRL3) ||
            (reg == PCI_MIC2_MC_U_BOX_CTRL) || (reg == PCI_MIC2_MC_U_BOX_STATUS) ||
            (reg == PCI_MIC2_MC_U_FIXED_CTR_A) || (reg == PCI_MIC2_MC_U_FIXED_CTR_B) ||
            (reg == PCI_MIC2_MC_U_FIXED_CTRL) || (reg == PCI_MIC2_MC_D_CTR0_A) ||
            (reg == PCI_MIC2_MC_D_CTR0_B) || (reg == PCI_MIC2_MC_D_CTR1_A) ||
            (reg == PCI_MIC2_MC_D_CTR1_B) || (reg == PCI_MIC2_MC_D_CTR2_A) ||
            (reg == PCI_MIC2_MC_D_CTR2_B) || (reg == PCI_MIC2_MC_D_CTR3_A) ||
            (reg == PCI_MIC2_MC_D_CTR3_B) || (reg == PCI_MIC2_MC_D_CTRL0) ||
            (reg == PCI_MIC2_MC_D_CTRL1) || (reg == PCI_MIC2_MC_D_CTRL2) ||
            (reg == PCI_MIC2_MC_D_CTRL3) || (reg == PCI_MIC2_MC_D_BOX_CTRL) ||
            (reg == PCI_MIC2_MC_D_BOX_STATUS) || (reg == PCI_MIC2_MC_D_FIXED_CTR_A) ||
            (reg == PCI_MIC2_MC_D_FIXED_CTR_B) || (reg == PCI_MIC2_MC_D_FIXED_CTRL)) {
            return 1;
        }
        break;
    case R2PCIE:
        if ((reg == PCI_MIC2_M2PCIE_CTR0_A) || (reg == PCI_MIC2_M2PCIE_CTR0_B) ||
            (reg == PCI_MIC2_M2PCIE_CTR1_A) || (reg == PCI_MIC2_M2PCIE_CTR1_B) ||
            (reg == PCI_MIC2_M2PCIE_CTR2_A) || (reg == PCI_MIC2_M2PCIE_CTR2_B) ||
            (reg == PCI_MIC2_M2PCIE_CTR3_A) || (reg == PCI_MIC2_M2PCIE_CTR3_B) ||
            (reg == PCI_MIC2_M2PCIE_CTRL0) || (reg == PCI_MIC2_M2PCIE_CTRL1) ||
            (reg == PCI_MIC2_M2PCIE_CTRL2) || (reg == PCI_MIC2_M2PCIE_CTRL3) ||
            (reg == PCI_MIC2_M2PCIE_BOX_CTRL) || (reg == PCI_MIC2_M2PCIE_BOX_STATUS)) {
            return 1;
        }
        break;
    case IRP:
        if ((reg == PCI_MIC2_IRP_CTR0) || (reg == PCI_MIC2_IRP_CTR1) ||
            (reg == PCI_MIC2_IRP_CTRL0) || (reg == PCI_MIC2_IRP_CTRL1) ||
            (reg == PCI_MIC2_IRP_BOX_CTRL) || (reg == PCI_MIC2_IRP_BOX_STATUS)) {
            return 1;
        }
        break;
    default:
...
static int allowed_skx(uint32_t reg)
{
    if (allowed_sandybridge(reg))
        ...
        if (((reg & 0xF00U) == 0x700U) || ((reg & 0xF00U) == 0xE00U) ||
            ((reg & 0xF00U) == 0xF00U) || (reg == MSR_PREFETCH_ENABLE) ||
            (reg == TSX_FORCE_ABORT) || ((reg & 0xA00U) == 0xA00U))
            return 1;
    }
    return 0;
}
...
static int allowed_pci_skx(PciDeviceType type, uint32_t reg)
{
    switch (type) {
    case HA:
        if ((reg == MSR_UNC_SKX_M2M_PMON_CTL0) || (reg == MSR_UNC_SKX_M2M_PMON_CTL1) ||
            (reg == MSR_UNC_SKX_M2M_PMON_CTL2) || (reg == MSR_UNC_SKX_M2M_PMON_CTL3) ||
            (reg == MSR_UNC_SKX_M2M_PMON_CTR0) || (reg == MSR_UNC_SKX_M2M_PMON_CTR1) ||
            (reg == MSR_UNC_SKX_M2M_PMON_CTR2) || (reg == MSR_UNC_SKX_M2M_PMON_CTR3) ||
            (reg == MSR_UNC_SKX_M2M_PMON_BOX_CTL) || (reg == MSR_UNC_SKX_M2M_PMON_BOX_STATUS))
            return 1;
        break;
    case IMC:
        if ((reg == PCI_UNC_SKX_MC_PMON_CTL0) || (reg == PCI_UNC_SKX_MC_PMON_CTL1) ||
            (reg == PCI_UNC_SKX_MC_PMON_CTL2) || (reg == PCI_UNC_SKX_MC_PMON_CTL3) ||
            (reg == PCI_UNC_SKX_MC_PMON_CTR0) || (reg == PCI_UNC_SKX_MC_PMON_CTR1) ||
            (reg == PCI_UNC_SKX_MC_PMON_CTR2) || (reg == PCI_UNC_SKX_MC_PMON_CTR3) ||
            (reg == PCI_UNC_SKX_MC_PMON_FIXED_CTL) || (reg == PCI_UNC_SKX_MC_PMON_FIXED_CTR) ||
            (reg == PCI_UNC_SKX_MC_PMON_BOX_CTL) || (reg == PCI_UNC_SKX_MC_PMON_BOX_STATUS))
            return 1;
        break;
    case QPI:
        if ((reg == MSR_UNC_SKX_UPI_PMON_CTL0) || (reg == MSR_UNC_SKX_UPI_PMON_CTL1) ||
            (reg == MSR_UNC_SKX_UPI_PMON_CTL2) || (reg == MSR_UNC_SKX_UPI_PMON_CTL3) ||
            (reg == MSR_UNC_SKX_UPI_PMON_CTR0) || (reg == MSR_UNC_SKX_UPI_PMON_CTR1) ||
            (reg == MSR_UNC_SKX_UPI_PMON_CTR2) || (reg == MSR_UNC_SKX_UPI_PMON_CTR3) ||
            (reg == MSR_UNC_SKX_UPI_PMON_BOX_CTL) || (reg == MSR_UNC_SKX_UPI_PMON_BOX_STATUS))
            return 1;
        break;
    case R3QPI:
        if ((reg == MSR_UNC_SKX_M3UPI_PMON_CTL0) || (reg == MSR_UNC_SKX_M3UPI_PMON_CTL1) ||
            (reg == MSR_UNC_SKX_M3UPI_PMON_CTL2) || (reg == MSR_UNC_SKX_M3UPI_PMON_CTR0) ||
            (reg == MSR_UNC_SKX_M3UPI_PMON_CTR1) || (reg == MSR_UNC_SKX_M3UPI_PMON_CTR2) ||
            (reg == MSR_UNC_SKX_M3UPI_PMON_BOX_CTL) || (reg == MSR_UNC_SKX_M3UPI_PMON_BOX_STATUS))
            return 1;
        break;
    default:
        break;
...
static int allowed_pci_icx(PciDeviceType type, uint32_t reg)
{
    switch (type) {
    case IMC:
        if ((reg == MMIO_ICX_IMC_BOX_CTRL) || (reg == MMIO_ICX_IMC_BOX_STATUS) ||
            (reg == MMIO_ICX_IMC_BOX_CTL0) || (reg == MMIO_ICX_IMC_BOX_CTL1) ||
            (reg == MMIO_ICX_IMC_BOX_CTL2) || (reg == MMIO_ICX_IMC_BOX_CTL3) ||
            (reg == MMIO_ICX_IMC_BOX_CTR0) || (reg == MMIO_ICX_IMC_BOX_CTR1) ||
            (reg == MMIO_ICX_IMC_BOX_CTR2) || (reg == MMIO_ICX_IMC_BOX_CTR3) ||
            (reg == MMIO_ICX_IMC_BOX_CLK_CTL) || (reg == MMIO_ICX_IMC_BOX_CLK_CTR) ||
            (reg == MMIO_ICX_IMC_FREERUN_DDR_RD) || (reg == MMIO_ICX_IMC_FREERUN_DDR_WR) ||
            (reg == MMIO_ICX_IMC_FREERUN_PMM_RD) || (reg == MMIO_ICX_IMC_FREERUN_PMM_RD) ||
            (reg == MMIO_ICX_IMC_FREERUN_DCLK))
            return 1;
    case HA:
        if ((reg == PCI_UNC_ICX_M2M_PMON_CTRL) || (reg == PCI_UNC_ICX_M2M_PMON_STATUS) ||
            (reg == PCI_UNC_ICX_M2M_PMON_CTL0) || (reg == PCI_UNC_ICX_M2M_PMON_CTL1) ||
            (reg == PCI_UNC_ICX_M2M_PMON_CTL2) || (reg == PCI_UNC_ICX_M2M_PMON_CTL3) ||
            (reg == PCI_UNC_ICX_M2M_PMON_CTR0) || (reg == PCI_UNC_ICX_M2M_PMON_CTR1) ||
            (reg == PCI_UNC_ICX_M2M_PMON_CTR2) || (reg == PCI_UNC_ICX_M2M_PMON_CTR3))
            return 1;
        break;
    case R3QPI:
        if ((reg == PCI_UNC_ICX_M3UPI_PMON_CTRL) || (reg == PCI_UNC_ICX_M3UPI_PMON_STATUS) ||
            (reg == PCI_UNC_ICX_M3UPI_PMON_CTL0) || (reg == PCI_UNC_ICX_M3UPI_PMON_CTL1) ||
            (reg == PCI_UNC_ICX_M3UPI_PMON_CTL2) || (reg == PCI_UNC_ICX_M3UPI_PMON_CTL3) ||
            (reg == PCI_UNC_ICX_M3UPI_PMON_CTR0) || (reg == PCI_UNC_ICX_M3UPI_PMON_CTR1) ||
            (reg == PCI_UNC_ICX_M3UPI_PMON_CTR2) || (reg == PCI_UNC_ICX_M3UPI_PMON_CTR3))
            return 1;
        break;
    case QPI:
        if ((reg == PCI_UNC_ICX_UPI_PMON_CTRL) || (reg == PCI_UNC_ICX_UPI_PMON_STATUS) ||
            (reg == PCI_UNC_ICX_UPI_PMON_CTL0) || (reg == PCI_UNC_ICX_UPI_PMON_CTL1) ||
            (reg == PCI_UNC_ICX_UPI_PMON_CTL2) || (reg == PCI_UNC_ICX_UPI_PMON_CTL3) ||
            (reg == PCI_UNC_ICX_UPI_PMON_CTR0) || (reg == PCI_UNC_ICX_UPI_PMON_CTR1) ||
            (reg == PCI_UNC_ICX_UPI_PMON_CTR2) || (reg == PCI_UNC_ICX_UPI_PMON_CTR3))
            return 1;
        break;
    default:
        break;
...
static int allowed_icx(uint32_t reg)
{
    if (allowed_sandybridge(reg))
        ...
        if (((reg & 0xF00U) == 0x700U) || ((reg & 0xF00U) == 0xA00U) ||
            ((reg & 0xF00U) == 0xB00U) || ((reg & 0xF00U) == 0xE00U) ||
            ((reg & 0xF00U) == 0xF00U) || (reg == MSR_PREFETCH_ENABLE) ||
            (reg == TSX_FORCE_ABORT) || (reg == MSR_PERF_METRICS))
            return 1;
    }
    return 0;
}
...
static int allowed_icl(uint32_t reg)
{
    if (allowed_sandybridge(reg))
        ...
        if ((reg == MSR_PREFETCH_ENABLE) || (reg == MSR_PERF_METRICS))
            return 1;
    }
    return 0;
}

static int allowed_spr(uint32_t reg) { return allowed_icx(reg); }

static int allowed_pci_spr(PciDeviceType type, uint32_t reg) { return 1; }
...
{
    if (((reg & 0xFFFFFFF0U) == 0xC0010000U) || ((reg & 0xFFFFFFF0U) == 0xC0010200U) ||
...
{
    if (((reg & 0xFFFFFFF0U) == 0xC0010000U) || ((reg & 0xFFFFFFF8U) == 0xC0010240U)) {
        return 1;
    } else {
        return 0;
    }
...
static int allowed_amd17(uint32_t reg)
{
    if ((reg >= 0xC0010200 && reg <= 0xC0010207) || (reg >= 0xC0010230 && reg <= 0xC001023B) ||
        (reg >= 0xC0010299 && reg <= 0xC001029B) || (reg >= 0xC00000E7 && reg <= 0xC00000E9) ||
        (reg >= 0xC0010240 && reg <= 0xC0010247) || (reg == 0xC0010015) || (reg == 0xC0010010) ||
        (reg == 0xC0000080)) {
        return 1;
    } else {
        return 0;
...
static int allowed_amd17_zen2(uint32_t reg)
{
    if (allowed_amd17(reg)) {
        return 1;
    } else if ((reg == 0xC0010208) || (reg == 0xC0010209) || (reg == 0xC001020A) ||
               (reg == 0xC001020B)) {
        return 1;
    }
    return 0;
...
static int allowed_amd19_zen3(uint32_t reg)
{
    if (allowed_amd17_zen2(reg)) {
        return 1;
    } else if ((reg == 0xC0000108) || (reg == 0x00000048) || (reg == 0x0000010B) ||
...
static int allowed_amd19_zen4c(uint32_t reg)
{
    if (allowed_amd19_zen3(reg)) {
        return 1;
    } else if ((reg == 0xC0000300) || (reg == 0xC0000301) || (reg == 0xC0000302) ||
               (reg == 0xC0000303)) {
        return 1;
    }
    return 0;
...
static void clientmem_read(AccessDataRecord *dRecord)
{
    uint64_t data      = 0;
...
static void clientmem_check(AccessDataRecord *dRecord)
{
    dRecord->errorcode = ERR_NOERROR;
    if (!isClientMem) {
        syslog(LOG_ERR, "ClientMem: Not available for this architecture\n");
...
static int servermem_getStartAddr(int socketId, int pmc_idx, void **mmap_addr)
{
    ...
    int ret =
        snprintf(sysfile, 999, "/sys/bus/pci/devices/0000:%.2x:00.1/config", pci_bus[socketId]);
    if (ret >= 0) {
        sysfile[ret] = '\0';
    } else {
        return -1;
    ...
    void *maddr =
        mmap(0, ICX_IMC_MMIO_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, pcihandle, addr & page_mask);
    if (maddr == MAP_FAILED) {
...
static void servermem_read(AccessDataRecord *dRecord)
{
    uint64_t data      = 0;
    uint32_t reg       = dRecord->reg;
...
static void servermem_freerun_read(AccessDataRecord *dRecord)
{
    uint64_t data      = 0;
    uint32_t reg       = dRecord->reg;
    uint32_t socketId  = dRecord->cpu;
    ...
    if (servermem_freerun_addrs && servermem_freerun_addrs[socketId] &&
        servermem_freerun_addrs[socketId][offset]) {
#ifdef DEBUG_LIKWID
        syslog(LOG_INFO, "ServerMem: Read S %d Box %d Reg 0x%x\n", socketId, offset, reg);
#endif
        switch (reg) {
...
static void servermem_write(AccessDataRecord *dRecord)
{
    uint32_t reg       = dRecord->reg;
    uint32_t socketId  = dRecord->cpu;
    uint32_t device    = dRecord->device;
...
#ifdef DEBUG_LIKWID
        syslog(LOG_INFO,
            "ServerMem: Write S %d Box %d Reg 0x%x Data 0x%x\n",
            socketId,
            offset,
            reg,
            dRecord->data);
...
static void servermem_freerun_write(AccessDataRecord *dRecord)
{
    uint32_t reg       = dRecord->reg;
    uint32_t socketId  = dRecord->cpu;
    uint32_t device    = dRecord->device;
...
static void servermem_check(AccessDataRecord *dRecord)
{
    dRecord->errorcode = ERR_NOERROR;
    if (!isServerMem) {
        syslog(LOG_ERR, "ServerMem: Not available for this architecture\n");
...
static void servermem_freerun_check(AccessDataRecord *dRecord)
{
    dRecord->errorcode = ERR_NOERROR;
    if (!isServerMem) {
        syslog(LOG_ERR, "ServerMem: Not available for this architecture\n");
 ...
 * node ID
 */
static int getBusFromSocketByNameDevid(
    const uint32_t socket, PciDevice *pcidev, int pathlen, char **filepath)
{
        ...
        if (strncmp(pDirent->d_name, "pci0", 4) == 0) {
            ...
            while ((pDirentInner = readdir(pDirInner)) != NULL) {
                if (strncmp(pDirentInner->d_name, "0000", 4) == 0) {
                    ...
                    memset(tmpPath, '\0', tmplen * sizeof(char));
                    sprintf(tmpPath,
                        "/sys/devices/%s/%s/device",
                        pDirent->d_name,
                        pDirentInner->d_name);
                    if (pcidev->path &&
                        strcmp(&(pDirentInner->d_name[strlen(pDirentInner->d_name) - 4]),
                            pcidev->path) != 0) {
                        continue;
                    }
                    fp = fopen(tmpPath, "r");
...
#ifdef DEBUG_LIKWID
    syslog(LOG_INFO,
        "PCI device %s (%s) for socket %d is on PCI bus 0x%02x",
        pcidev->likwid_name,
        pcidev->path,
        socket,
        bus_id);
...
static int getBusFromSocketByDevid(
    const uint32_t socket, PciDevice *pcidev, int pathlen, char **filepath)
{
    int ret            = 0;
    ...
    if (filepath && *filepath && pathlen > 0 && candidate >= 0 && candidates[candidate].bus >= 0 &&
        candidates[candidate].bus <= 0xff) {
        snprintf(*filepath,
            pathlen - 1,
            "/proc/bus/pci/%02x/%02x.%01x",
            candidates[candidate].bus,
            PCI_SLOT(candidates[candidate].devfn),
            PCI_FUNC(candidates[candidate].devfn));
    }
...
static int getBusFromSocketByNameDevidNode(
    const uint32_t socket, PciDevice *pcidev, int pathlen, char **filepath)
{
    FILE *fpipe  = NULL;
    int isSNC    = 0;
...
static void pci_read(AccessDataRecord *dRecord)
{
    ...
    if (!FD_PCI[socketId][device]) {
        ...
        if (sid == socketId) {
#ifdef DEBUG_LIKWID
            syslog(LOG_ERR,
                "Open device file %s for device %s (%s) on socket %u",
                pcipath,
                pci_types[pci_devices_daemon[device].type].name,
                pci_devices_daemon[device].name,
                socketId);
#endif
            ...
            if (FD_PCI[socketId][device] < 0) {
                syslog(LOG_ERR,
                    "Failed to open device file %s for device %s (%s) on socket %u",
                    pcipath,
                    pci_types[pci_devices_daemon[device].type].name,
                    pci_devices_daemon[device].name,
                    socketId);
    ...
    if (!isPCI64) {
        if (FD_PCI[socketId][device] > 0 &&
            pread(FD_PCI[socketId][device], &data, sizeof(data), reg) != sizeof(data)) {
            syslog(LOG_ERR,
                "Failed to read data from pci device file %s for device %s (%s) on socket %u",
                pcipath,
                pci_types[pci_devices_daemon[device].type].name,
                pci_devices_daemon[device].name,
                socketId);
        ...
        if (FD_PCI[socketId][device] > 0 &&
            pread(FD_PCI[socketId][device], &data64, sizeof(data64), reg) != sizeof(data64)) {
            syslog(LOG_ERR,
                "Failed to read data from pci device file %s for device %s (%s) on socket %u",
                pcipath,
                pci_types[pci_devices_daemon[device].type].name,
...
static void pci_write(AccessDataRecord *dRecord)
{
    ...
    if (!isPCI64) {
        if (FD_PCI[socketId][device] > 0 &&
            pwrite(FD_PCI[socketId][device], &data, sizeof data, reg) != sizeof data) {
            syslog(LOG_ERR,
                "Failed to write data to pci device file %s for device %s (%s) on socket %u",
                pci_filepath,
                pci_types[pci_devices_daemon[device].type].name,
                pci_devices_daemon[device].name,
                socketId);
        ...
        if (FD_PCI[socketId][device] > 0 &&
            pwrite(FD_PCI[socketId][device], &data64, sizeof data64, reg) != sizeof data64) {
            syslog(LOG_ERR,
                "Failed to write data to pci device file %s for device %s (%s) on socket %u",
                pci_filepath,
                pci_types[pci_devices_daemon[device].type].name,
...
static void handle_record_default(AccessDataRecord *record)
{
    if (record->type == DAEMON_READ) {
        if (record->device == MSR_DEV) {
            ...
            if (record->device >= MMIO_IMC_DEVICE_0_CH_0 &&
                record->device <= MMIO_IMC_DEVICE_3_CH_1) {
                servermem_read(record);
            } else if (record->device >= MMIO_IMC_DEVICE_0_FREERUN &&
                       record->device <= MMIO_IMC_DEVICE_3_FREERUN) {
...
static void msr_read_spr(AccessDataRecord *dRecord)
{
    uint64_t data;
    uint32_t cpu       = dRecord->cpu;
    uint32_t reg       = dRecord->reg;
...
static void msr_write_spr(AccessDataRecord *dRecord)
{
    uint32_t cpu       = dRecord->cpu;
    uint32_t reg       = dRecord->reg;
    uint64_t data      = dRecord->data;
...
static void spr_read_global(AccessDataRecord *record)
{
    ...
    if (perfmon_discovery && record->cpu >= 0 && record->cpu < perfmon_discovery->num_sockets &&
        record->device == MSR_UBOX_DEVICE) {
        PerfmonDiscoverySocket *cur = &perfmon_discovery->sockets[record->cpu];
        if (cur && cur->socket_id == record->cpu && cur->global.global_ctl &&
            cur->global.access_type == ACCESS_TYPE_MSR && socket_map[record->cpu] >= 0) {
            ...
            } else if ((record->reg >= FAKE_UNC_GLOBAL_STATUS0) &&
                       (record->reg <= FAKE_UNC_GLOBAL_STATUS3) &&
                       (cur->global.num_status > (record->reg - FAKE_UNC_GLOBAL_STATUS0))) {
                msr_record.reg = cur->global.global_ctl + cur->global.status_offset +
                                 (record->reg - FAKE_UNC_GLOBAL_STATUS0);
            }
    ...
    if (perfmon_discovery && record->cpu >= 0 && record->cpu < perfmon_discovery->num_sockets) {
        ...
        if (cur->socket_id == record->cpu && cur->units[record->device].num_regs > 0) {
            ...
            case FAKE_UNC_UNIT_CTRL:
                ...
                case ACCESS_TYPE_PCI:
                    if ((record->device >= PCI_HA_DEVICE_0 && record->device <= PCI_HA_DEVICE_31) ||
                        (record->device >= PCI_R3QPI_DEVICE_LINK_0 &&
                            record->device <= PCI_R3QPI_DEVICE_LINK_5) ||
                        (record->device >= PCI_QPI_DEVICE_PORT_0 &&
                            record->device <= PCI_QPI_DEVICE_PORT_5)) {
            ...
            case FAKE_UNC_CTRL3:
                ...
                case ACCESS_TYPE_MMIO:
                    if ((record->device >= MMIO_IMC_DEVICE_0_CH_0 &&
                            record->device <= MMIO_IMC_DEVICE_1_CH_7) ||
                        (record->device >= MMIO_HBM_DEVICE_0 &&
                            record->device <= MMIO_HBM_DEVICE_31) ||
                        (record->device >= PCI_R2PCIE_DEVICE0 &&
                            record->device <= PCI_R2PCIE_DEVICE31)) {
                        record->data = (uint32_t)*(
            ...
            case FAKE_UNC_UNIT_STATUS:
                ...
                case ACCESS_TYPE_PCI:
                    if ((record->device >= PCI_HA_DEVICE_0 && record->device <= PCI_HA_DEVICE_31) ||
                        ...
                        *((uint32_t *)(unit->io_addr + unit->mmap_offset + unit->status_offset +
                                       sizeof(uint32_t))) = (uint32_t)(record->data >> 32);
                    } else {
                        *((uint64_t *)(unit->io_addr + unit->mmap_offset + unit->status_offset)) =
                ...
                case ACCESS_TYPE_MMIO:
                    if ((record->device >= MMIO_IMC_DEVICE_0_CH_0 &&
                        ...
                        (record->device >= PCI_R2PCIE_DEVICE0 &&
                            record->device <= PCI_R2PCIE_DEVICE31)) {
                        *((uint32_t *)(unit->io_addr + unit->mmap_offset + unit->ctrl_offset +
                                       (sizeof(uint32_t) * offset))) = (uint32_t)record->data;
                    } else {
                        *((uint64_t *)(unit->io_addr + unit->mmap_offset + unit->ctrl_offset +
                                       (reg_offset * offset))) = (uint64_t)record->data;
                    }
                    record->errorcode = ERR_NOERROR;
                    break;
                case ACCESS_TYPE_PCI:
                ...
                if (unit->fixed_ctrl_offset != 0) {
                    if (unit->access_type == ACCESS_TYPE_MSR) {
                        ...
                        *((uint32_t *)(unit->io_addr + unit->mmap_offset +
                                       unit->fixed_ctrl_offset)) = (uint32_t)record->data;
                ...
                if (unit->fixed_ctr_offset != 0) {
                    if (unit->access_type == ACCESS_TYPE_MSR) {
                        ...
                        *((uint64_t *)(unit->io_addr + unit->mmap_offset +
                                       unit->fixed_ctr_offset)) = record->data;
                        record->errorcode                       = ERR_NOERROR;
                    }
                }
                break;
...
{
    if (record->device == MSR_UBOX_DEVICE &&
        ((record->reg == FAKE_UNC_GLOBAL_CTRL) || ((record->reg >= FAKE_UNC_GLOBAL_STATUS0) &&
                                                      (record->reg <= FAKE_UNC_GLOBAL_STATUS8)))) {
...
int main(void)
{
...

    {
        ...
        case P6_FAMILY:
            ...
            if ((model == SANDYBRIDGE) || (model == IVYBRIDGE)) {
                ...
                isPCIUncore = 1;
            } else if ((model == HASWELL) || (model == HASWELL_M1) || (model == HASWELL_M2) ||
                       (model == BROADWELL) || (model == BROADWELL_E3) || (model == SKYLAKE1) ||
                       (model == SKYLAKE2) || (model == KABYLAKE1) || (model == KABYLAKE2) ||
                       (model == COMETLAKE1) || (model == COMETLAKE2) || (model == CANNONLAKE)) {
                allowed     = allowed_sandybridge;
                isClientMem = 1;
            } else if ((model == ICELAKE1) || (model == ICELAKE2) || (model == ROCKETLAKE) ||
                       (model == TIGERLAKE1) || (model == TIGERLAKE2)) {
                allowed     = allowed_icl;
                isClientMem = 1;
                ...
                allowedPci             = allowed_pci_spr;
                isIntelUncoreDiscovery = 1;
            } else if ((model == ATOM_SILVERMONT_C) || (model == ATOM_SILVERMONT_E) ||
                       (model == ATOM_SILVERMONT_Z1) || (model == ATOM_SILVERMONT_Z2) ||
                       (model == ATOM_SILVERMONT_F) || (model == ATOM_SILVERMONT_AIR)) {
                allowed = allowed_silvermont;
            } else if ((model == XEON_PHI_KNL) || (model == XEON_PHI_KML)) {
                allowed     = allowed_knl;
                isPCIUncore = 1;
        ...
        default:
            syslog(LOG_ERR,
                "ERROR - [%s:%d] - Unsupported processor. Exiting!  \n",
                __FILE__,
                __LINE__);
            exit(EXIT_FAILURE);
        }
...

    {
        ...
        for (uint32_t i = 0; i < avail_cpus; i++) {
            ...
            if (FD_MSR[i] < 0) {
                syslog(LOG_ERR,
                    "Failed to open device file %s: %s, trying /dev/msr%d",
                    msr_file_name,
                    strerror(errno),
                    i);
                ...
                if (FD_MSR[i] < 0) {
                    syslog(LOG_ERR,
                        "Failed to open device file %s: %s.",
                        msr_file_name,
                        strerror(errno));
                }
            }
        ...
        if (isPCIUncore) {
            ...
            for (int i = 1; i < MAX_NUM_PCI_DEVICES; i++) {
                if (pci_devices_daemon && pci_devices_daemon[i].path &&
                    strlen(pci_devices_daemon[i].path) > 0) {
                    ...
                    if (socket_id == 0) {
                        ...
                        syslog(LOG_ERR,
                            "Device %s not found, excluded it from device list\n",
...
LOOP:
    while (1) {
        ...
        LOG_AND_EXIT_IF_ERROR(
            write(connfd, (void *)&dRecord, sizeof(AccessDataRecord)), write failed);
    }
