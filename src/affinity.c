...
static int treeFillNextEntries(TreeNode *tree, int *processorIds, int startidx, int socketId,
    int coreOffset, int coreSpan, int numberOfEntries)
{
...
static int create_lookups()
{
    ...
    for (unsigned pu_idx = 0; pu_idx < cputopo->numHWThreads; pu_idx++) {
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "T %d T2C %d T2S %d T2D %d T2LLC %d T2M %d",
            apicId,
...
static int affinity_addNodeDomain(AffinityDomain *domain, int *help)
{
    ...
    if (cputopo) {
        ...
        domain->tag[1]             = '\0';
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Affinity domain N: %d HW threads on %d cores",
            domain->numberOfProcessors,
            domain->numberOfCores);
        return 0;
    }
    return -EINVAL;
}
...
static int affinity_addSocketDomain(int socket, AffinityDomain *domain, int *help)
{
    ...
    if (cputopo) {
        ...
        domain->numberOfCores =
            affinity_countSocketCores(domain->numberOfProcessors, domain->processorList, help);
        snprintf(domain->tag, 9, "S%d", socket);
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Affinity domain S%d: %d HW threads on %d cores",
            socket,
            domain->numberOfProcessors,
            domain->numberOfCores);
        return 0;
...
static int affinity_addDieDomain(int socket, int die, AffinityDomain *domain, int *help)
{
    ...
    if (cputopo) {
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Affinity domain D%d: %d HW threads on %d cores",
            dieId,
            domain->numberOfProcessors,
            domain->numberOfCores);
        return 0;
...
static int affinity_addCacheDomain(int socket, int cacheId, AffinityDomain *domain, int *help)
{
    ...
    if (cputopo && cputopo->numCacheLevels > 0) {
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Affinity domain C%d: %d HW threads on %d cores",
            cid,
            domain->numberOfProcessors,
            domain->numberOfCores);
        return 0;
...
static int _affinity_addMemoryDomain(int nodeId, AffinityDomain *domain, int *help)
{
    ...
    if (cputopo && numatopo) {
        if (nodeId >= 0 && (uint32_t)nodeId < numatopo->numberOfNodes) {
            domain->processorList =
                malloc(numatopo->nodes[nodeId].numberOfProcessors * sizeof(int));
            if (!domain->processorList) {
                return -ENOMEM;
            }
            ...
            for (unsigned i = 0; i < numatopo->nodes[nodeId].numberOfProcessors; i++) {
                for (unsigned j = 0; j < cputopo->numHWThreads; j++) {
                    if (cputopo->threadPool[j].apicId == numatopo->nodes[nodeId].processors[i] &&
                        cputopo->threadPool[j].inCpuSet == 1) {
                        domain->processorList[num_hwthreads++] =
                            (int)numatopo->nodes[nodeId].processors[i];
                    }
                }
            }
        ...
        if (nodeId >= 0 && nodeId < cudatopo->numDevices) {
            ...
            if (cudadev->numaNode >= 0) {
                ...
                if (err == 0) {
                    ...
                    DEBUG_PRINT(DEBUGLEV_DEVELOP,
                        "Affinity domain %s: %d HW threads on %d cores",
                        domain->tag,
                        domain->numberOfProcessors,
                        domain->numberOfCores);
                    return 0;
    ...
    if (cputopo && numatopo && rocmtopo) {
        if (nodeId >= 0 && nodeId < rocmtopo->numDevices) {
            ...
            if (rocmdev->numaNode >= 0) {
                ...
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    "Skipping affinity domain G%d because NUMA node unknown",
                    nodeId);
            }
        }
    }
...
int affinity_init(void)
{
    ...
    DEBUG_PRINT(DEBUGLEV_DEVELOP,
        "Affinity: CPU die domains %d",
        (cputopo->numDies > 0 ? cputopo->numDies : cputopo->numSockets));
    ...
    if (doCudaDomains) {
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Affinity: CUDA domains %d (%d device(s))",
            numCudaDomains,
            cudatopo->numDevices);
    ...
    if (doRocmDomains) {
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "Affinity: ROCm domains %d (%d device(s))",
            numRocmDomains,
            rocmtopo->numDevices);
    ...
    if (!domains) {
        fprintf(stderr,
            "No more memory for %ld bytes for array of affinity domains\n",
    ...
    affinityDomains.numberOfProcessorsPerSocket =
        cputopo->numCoresPerSocket * cputopo->numThreadsPerCore;
...
int affinity_processGetProcessorId(void)
{
    int ret;
    cpu_set_t cpu_set;
    CPU_ZERO(&cpu_set);
...
int affinity_threadGetProcessorId(void)
{
    cpu_set_t cpu_set;
    CPU_ZERO(&cpu_set);
...
void affinity_pinProcesses(int cpu_count, const int *processorIds)
{
    int i;
    cpu_set_t cpuset;
...
const AffinityDomain *affinity_getDomain(char *domain)
{
...
void affinity_printDomains(void)
{
    for (int i = 0; i < affinity_numberOfDomains; i++) {
        printf("Domain %d:\n", i);
...
AffinityDomains_t get_affinityDomains(void) { return &affinityDomains; }
