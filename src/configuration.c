...
static int default_configuration(void)
{
    ...
    if (!access(fptr, X_OK)) {
        ...
        fprintf(stderr,
            "Found access daemon at %s but it is not executable, using compiled in daemon path.\n",
            fptr);
        if (fptr)
            free(fptr);
        goto use_hardcoded;
...
use_hardcoded:
    ...
    if (!access(filename, X_OK)) {
        ...
        if (getenv("LIKWID_NO_ACCESS") == NULL) {
            ERROR_PRINT("Unable to get path to access daemon. Maybe your PATH environment variable "
                        "does not contain the folder where you installed it or the file was moved "
                        "away / not copied to that location?");
            return -1;
        }
    }
    return 0;
...
int init_configuration(void)
{
    ...
    while (fgets(line, 512, fp) != NULL) {
        ...
        if (strcmp(name, "topology_file") == 0 &&
            /*don't overrule user request*/ !(customtopo && !access(customtopo, R_OK))) {
            ...
            if (strcmp(value, "daemon") == 0) {
                ...
                config.daemonMode = ACCESSMODE_PERF;
#else
                ERROR_PRINT("Invalid access mode 'perf_event'. Library not built with 'perf_event' "
                            "support");
#endif
            }
