...
static void cpuFeatures_update(int cpu)
{
    ...
    if ((cpuid_info.model == CORE2_45) || (cpuid_info.model == CORE2_65)) {
        ...
        TEST_FLAG_INV(FEAT_IP_PREFETCHER, 39);
    } else if ((cpuid_info.model == NEHALEM) || (cpuid_info.model == NEHALEM_BLOOMFIELD) ||
               (cpuid_info.model == NEHALEM_LYNNFIELD) || (cpuid_info.model == NEHALEM_WESTMERE) ||
               (cpuid_info.model == NEHALEM_WESTMERE_M) || (cpuid_info.model == NEHALEM_EX) ||
               (cpuid_info.model == WESTMERE_EX) || (cpuid_info.model == ATOM_SILVERMONT_E) ||
               ...
               (cpuid_info.model == ATOM_SILVERMONT_GOLD) || (cpuid_info.model == SANDYBRIDGE) ||
               (cpuid_info.model == SANDYBRIDGE_EP) || (cpuid_info.model == IVYBRIDGE) ||
               (cpuid_info.model == IVYBRIDGE_EP) || (cpuid_info.model == HASWELL) ||
               (cpuid_info.model == HASWELL_M1) || (cpuid_info.model == HASWELL_M2) ||
               (cpuid_info.model == HASWELL_EP) || (cpuid_info.model == BROADWELL) ||
               (cpuid_info.model == BROADWELL_E3) || (cpuid_info.model == BROADWELL_D) ||
               (cpuid_info.model == BROADWELL_E) || (cpuid_info.model == SKYLAKE1) ||
               (cpuid_info.model == SKYLAKE2) || (cpuid_info.model == SKYLAKEX) ||
               (cpuid_info.model == KABYLAKE1) || (cpuid_info.model == KABYLAKE2) ||
               (cpuid_info.model == CANNONLAKE) || (cpuid_info.model == COMETLAKE1) ||
               (cpuid_info.model == COMETLAKE2)) {
        TEST_FLAG_INV(FEAT_TURBO_MODE, 38);
    }

    if ((cpuid_info.model == NEHALEM) || (cpuid_info.model == NEHALEM_BLOOMFIELD) ||
        (cpuid_info.model == NEHALEM_LYNNFIELD) || (cpuid_info.model == NEHALEM_WESTMERE) ||
        (cpuid_info.model == NEHALEM_WESTMERE_M) || (cpuid_info.model == NEHALEM_EX) ||
        (cpuid_info.model == WESTMERE_EX) || (cpuid_info.model == SANDYBRIDGE) ||
        (cpuid_info.model == SANDYBRIDGE_EP) || (cpuid_info.model == IVYBRIDGE) ||
        (cpuid_info.model == IVYBRIDGE_EP) || (cpuid_info.model == HASWELL) ||
        (cpuid_info.model == HASWELL_M1) || (cpuid_info.model == HASWELL_M2) ||
        (cpuid_info.model == HASWELL_EP) || (cpuid_info.model == BROADWELL) ||
        ...
        (cpuid_info.model == ROCKETLAKE) || (cpuid_info.model == ICELAKEX1) ||
        (cpuid_info.model == ICELAKEX2) || (cpuid_info.model == SAPPHIRERAPIDS) ||
        (cpuid_info.model == ATOM_SILVERMONT_GOLD) || (cpuid_info.model == CANNONLAKE) ||
        (cpuid_info.model == COMETLAKE1) || (cpuid_info.model == COMETLAKE2)) {
    ...
    if ((cpuid_info.model == XEON_PHI_KNL) || (cpuid_info.model == XEON_PHI_KML)) {
        ret = HPMread(cpu, MSR_DEV, MSR_PREFETCH_ENABLE, &flags);
        if (ret != 0) {
            fprintf(stderr,
                "Cannot read register 0x%X on cpu %d: err %d\n",
...
void cpuFeatures_init(void)
{
    ...
    for (unsigned i = 0; i < cpuid_topology.numHWThreads; i++) {
        ...
        if (ret != 0) {
            ERROR_PRINT("Cannot get access to register CPU feature register on CPU %d",
...
void cpuFeatures_print(int cpu)
{
    ...
    for (int i = 0; i < CPUFEATURES_MAX; i++) {
        if ((cpuid_info.model != CORE2_45) && (cpuid_info.model != CORE2_65) &&
            ((i == FEAT_FERR_MULTIPLEX) || (i == FEAT_DYN_ACCEL) || (i == FEAT_SPEEDSTEP_LOCK) ||
                (i == FEAT_TM2))) {
            continue;
        }
...
int cpuFeatures_enable(int cpu, CpuFeature type, int print)
{
    int ret;
    uint64_t flags;
    ...
    if ((cpuid_info.model == NEHALEM) || (cpuid_info.model == NEHALEM_BLOOMFIELD) ||
        ...
        (cpuid_info.model == KABYLAKE1) || (cpuid_info.model == KABYLAKE2) ||
        (cpuid_info.model == CANNONLAKE) || (cpuid_info.model == COMETLAKE1) ||
        (cpuid_info.model == COMETLAKE2) || (cpuid_info.model == ICELAKE1) ||
        (cpuid_info.model == ICELAKE2) || (cpuid_info.model == SAPPHIRERAPIDS) ||
        (cpuid_info.model == GRANITERAPIDS) || (cpuid_info.model == SIERRAFORREST) ||
        (cpuid_info.model == ROCKETLAKE) || (cpuid_info.model == ICELAKEX1) ||
        (cpuid_info.model == ICELAKEX2) || (cpuid_info.model == ATOM_SILVERMONT_GOLD)) {
    ...
    if ((cpuid_info.model == XEON_PHI_KNL) || (cpuid_info.model == XEON_PHI_KML)) {
        reg        = MSR_PREFETCH_ENABLE;
        knlOffsets = 1;
        if (type == FEAT_CL_PREFETCHER || type == FEAT_IP_PREFETCHER) {
            fprintf(
                stderr, "CL_PREFETCHER and IP_PREFETCHER not available on Intel Xeon Phi (KNL)");
            return 0;
        }
    }
    ...
    if (ret != 0) {
        fprintf(stderr,
            "Cannot read register 0x%X for CPU %d to activate feature %s\n",
            reg,
            cpu,
...
int cpuFeatures_disable(int cpu, CpuFeature type, int print)
{
    int ret;
    uint64_t flags;
    uint32_t reg   = MSR_IA32_MISC_ENABLE;
