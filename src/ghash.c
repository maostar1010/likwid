...
static gpointer g_malloc(gsize n_bytes)
{
    if (G_LIKELY(n_bytes)) {
        gpointer mem;
...
static gpointer g_malloc0(gsize n_bytes)
{
    if (G_LIKELY(n_bytes)) {
        gpointer mem;
...
static gpointer g_malloc0_n(gsize n_blocks, gsize n_block_bytes)
{
...
static gpointer g_memdup(gconstpointer mem, guint byte_size)
{
    gpointer new_mem;
...
static inline guint g_hash_table_lookup_node(
    GHashTable *hash_table, gconstpointer key, guint *hash_return)
{
    guint node_index;
    guint node_hash;
    guint hash_value;
...
int g_hash_table_remove(GHashTable *hash_table, gpointer key)
{
    int hashret = 0;
    if (key) {
        int *key_ptr = (int *)key;
...
{
    if (!notify ||
        (hash_table->key_destroy_func == NULL && hash_table->value_destroy_func == NULL)) {
...
static gboolean g_hash_table_insert_node(GHashTable *hash_table, guint node_index, guint key_hash,
    gpointer new_key, gpointer new_value, gboolean keep_new_key, gboolean reusing_key)
{
    ...
    if (G_UNLIKELY(
            hash_table->keys == hash_table->values && hash_table->keys[node_index] != new_value))
...
gpointer g_hash_table_lookup(GHashTable *hash_table, gconstpointer key)
{
    ...
    return HASH_IS_REAL(hash_table->hashes[node_index]) ? hash_table->values[node_index] : NULL;
}

static gboolean g_hash_table_insert_internal(
    GHashTable *hash_table, gpointer key, gpointer value, gboolean keep_new_key)
{
    ...
    return g_hash_table_insert_node(
        hash_table, node_index, key_hash, key, value, keep_new_key, FALSE);
}
...
guint g_hash_table_size(GHashTable *hash_table) { return hash_table->nnodes; }
