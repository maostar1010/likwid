...
struct perf_event_config_format {
    enum { INVALID = 0, CONFIG, CONFIG1, CONFIG2 } reg;
    int start;
    int end;
};
...
int perfmon_setupCountersThread_perfevent(int thread_id, PerfmonEventSet *eventSet)
{
    int ret;
        ...
        if (ret == 0) {
            ...
            if (!is_uncore) {
                ...
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    ...
                    "inherit=%d exclusive=%d config1=0x%llX config2=0x%llX",
                    cpu_id,
                    curpid,
                    allflags,
                    attr.type,
