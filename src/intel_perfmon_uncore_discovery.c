    ...
    if (sysfs_pci_devices == NULL) {
        ...
        while ((ep = readdir(dp)) != NULL) {
            ...
            if (numa_node >= 0) {
                int cpuid = -1;
                ret =
                    snprintf(filename, 1023, "/sys/devices/system/node/node%d/cpulist", numa_node);
            ...
            struct pci_dev *tmp =
                realloc(sysfs_pci_devices, (num_sysfs_pci_devices + 1) * sizeof(struct pci_dev));
            if (!tmp) {
                break;
            }
...
static void print_unit(PciDeviceIndex idx, PerfmonDiscoveryUnit *unit)
{
    ...
    if (name != NULL) {
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "PCIIDX %d Access %s NumRegs %d ID %d Type %s(%d) box_ctl 0x%lX ctrl_offset 0x%lX "
            "ctr_offset 0x%lX mmap_addr 0x%lX mmap_offset 0x%lX",
            idx,
    ...
    for (int i = 0; i < num_sysfs_pci_devices; i++) {
        ...
        if (myid == unit->box_id) {
            unit->box_ctl = dev->domain << UNCORE_DISCOVERY_PCI_DOMAIN_OFFSET |
                            ...
                            mydevfn << UNCORE_DISCOVERY_PCI_DEVFN_OFFSET | unit->box_ctl;
        }
    }
    if (dev)
        free(dev);
...
int perfmon_uncore_discovery(int model, PerfmonDiscovery **perfmon)
{
    ...
    for (int i = 0; i < num_sysfs_pci_devices; i++) {
        ...
        while ((dvsec = pci_find_next_ext_capability(dev, dvsec, UNCORE_EXT_CAP_ID_DISCOVERY))) {
            ...
            if (ret < 0) {
                ERROR_PRINT("Failed to read DVSEC offset from device %.04x:%.02x:%.02x.%.01x",
                    dev->domain,
                    dev->bus,
                    dev->device,
                    dev->func);
            ...
            if (entry_id == UNCORE_DISCOVERY_DVSEC_ID_PMON) {
                ...
                if (ret < 0) {
                    ERROR_PRINT("Failed to read DIR from device %.04x:%.02x:%.02x.%.01x",
                        dev->domain,
                        dev->bus,
                        dev->device,
                        dev->func);
                ...
                if (ret < 0) {
                    ERROR_PRINT("Failed to read BAR offset from device %.04x:%.02x:%.02x.%.01x",
                        dev->domain,
                        dev->bus,
                        dev->device,
                        dev->func);
                ...
                if (io_addr == MAP_FAILED) {
                    ERROR_PRINT("Failed to mmap device %.04x:%.02x:%.02x.%.01x",
                        dev->domain,
                        dev->bus,
                        dev->device,
                        dev->func);
                ...
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    "Device %.04x:%.02x:%.02x.%.01x usable with %d units",
                    dev->domain,
                    dev->bus,
                    dev->device,
                    dev->func,
                ...
                for (int i = 0; i < global.max_units; i++) {
                    ...
                    if (idx >= 0 && idx < MAX_NUM_PCI_DEVICES) {
                        ...
                        case GRANITERAPIDS:
                            ...
                            } else if (unit.box_type == SPR_DEVICE_ID_iMC ||
                                       (unit.box_type == SPR_DEVICE_ID_HBM &&
                        ...
                        cur->units[idx].mmap_offset =
                            cur->units[idx].box_ctl - cur->units[idx].mmap_addr;
        ...
        for (int i = 0; i < perfmon->num_sockets; i++) {
            for (int j = 0; j < MAX_NUM_PCI_DEVICES; j++) {
                if (perfmon->sockets[i].units[j].io_addr) {
                    munmap((void *)perfmon->sockets[i].units[j].io_addr,
                        perfmon->sockets[i].units[j].mmap_size);
