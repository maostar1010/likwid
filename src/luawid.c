...
static int lua_likwid_getPowerInfo(lua_State *L)
{
    ...
    lua_pushstring(L, "domains");
    lua_newtable(L);
    for (int i = 0; i < power->numDomains; i++) {
...
static void catch_sigchild(int signo) { (void)signo; }
...
static int lua_likwid_setVerbosity(lua_State *L)
{
    ...
    luaL_argcheck(L,
        (verbosity >= 0 && verbosity <= DEBUGLEV_DEVELOP),
        -1,
...
static int lua_likwid_access(lua_State *L)
{
    ...
    if (!perm) {
        ...
        for (size_t i = 0; i < strlen(perm); i++) {
            if (perm[i] == 'r') {
                flags |= R_OK;
            } else if (perm[i] == 'w') {
                flags |= W_OK;
...
static int lua_likwid_nvMarkerRegionGpulist(lua_State *L)
{
    ...
    regionGPUs = nvmon_getGpulistOfRegion(region - 1, cudatopo->numDevices, gpulist);
    if (regionGPUs > 0) {
        lua_newtable(L);
...
static int lua_likwid_markerRegionGpulist_rocm(lua_State *L)
{
    ...
    regionGPUs = rocmon_getGpulistOfRegion(region - 1, rocmtopo->numDevices, gpulist);
    if (regionGPUs > 0) {
        lua_newtable(L);
        for (i = 0; i < regionGPUs; i++) {
            lua_pushinteger(L, i + 1);
...
static int lua_likwid_markerRegionResult_rocm(lua_State *L)
{
    ...
    int event  = lua_tointeger(L, -2);
    int gpu    = lua_tointeger(L, -1);
    lua_pushnumber(L, rocmon_getResultOfRegionGpu(region - 1, event - 1, gpu - 1));
    return 1;
}
...
static int lua_likwid_markerRegionMetric_rocm(lua_State *L)
{
    ...
    int metric = lua_tointeger(L, -2);
    int gpu    = lua_tointeger(L, -1);
    lua_pushnumber(L, rocmon_getMetricOfRegionGpu(region - 1, metric - 1, gpu - 1));
    return 1;
}
...
static int lua_likwid_initSysFeatures(lua_State *L)
{
    int err = 0;
...
static int lua_likwid_finalizeSysFeatures(lua_State *L)
{
    (void)L;
    if (sysfeatures_inititalized) {
...
static int lua_likwid_getSysFeatureList(lua_State *L)
{
    if (!sysfeatures_inititalized) {
        lua_newtable(L);
        return 1;
...
static int lua_likwid_getSysFeature(lua_State *L)
{
    if (!sysfeatures_inititalized)
...
static int lua_likwid_setSysFeature(lua_State *L)
{
    if (!sysfeatures_inititalized)
        return luaL_error(L, "likwid sysfeatures not initialized");
...
static int lua_likwid_getAvailableDevices(lua_State *L) { return lua_likwid_getDevices(L, false); }

static int lua_likwid_getAllDevices(lua_State *L) { return lua_likwid_getDevices(L, true); }
...
static int lua_likwid_createDevicesFromString(lua_State *L)
{
...
static int lua_likwid_createDevice(lua_State *L)
{
    ...
    if (lua_isinteger(L, 2)) {
        ...
        return luaL_error(
            L, "ID must be int or string, found: %s", lua_typename(L, lua_type(L, 2)));
    }
...
static int lua_likwid_sysFeaturesSupported(lua_State *L)
{
...
int __attribute__((visibility("default"))) luaopen_liblikwid(lua_State *L)
{
    ...
    lua_register(L, "likwid_cpuFeaturesPrint", lua_likwid_cpuFeatures_print);
    lua_register(L, "likwid_cpuFeaturesName", lua_likwid_cpuFeatures_name);
    ...
    lua_register(L, "likwid_getCudaEventsAndCounters", lua_likwid_getCudaEventsAndCounters);
    ...
    lua_register(L, "likwid_getRocmEventsAndCounters", lua_likwid_getRocmEventsAndCounters);
    lua_register(L, "likwid_getRocmGroups", lua_likwid_getRocmGroups);
    lua_register(L, "likwid_gpustr_to_gpulist_rocm", lua_likwid_gpustr_to_gpulist_rocm);
    ...
    lua_register(L, "likwid_getNameOfCounter_rocm", lua_likwid_getNameOfCounter_rocm);
    lua_register(L, "likwid_getNameOfMetric_rocm", lua_likwid_getNameOfMetric_rocm);
    lua_register(L, "likwid_getNameOfGroup_rocm", lua_likwid_getNameOfGroup_rocm);
    lua_register(L, "likwid_readMarkerFile_rocm", lua_likwid_markerFile_read_rocm);
    lua_register(L, "likwid_markerFile_destroy_rocm", lua_likwid_markerFile_destroy_rocm);
    lua_register(L, "likwid_markerNumRegions_rocm", lua_likwid_markerNumRegions_rocm);
    lua_register(L, "likwid_markerRegionGroup_rocm", lua_likwid_markerRegionGroup_rocm);
    lua_register(L, "likwid_markerRegionTag_rocm", lua_likwid_markerRegionTag_rocm);
    lua_register(L, "likwid_markerRegionEvents_rocm", lua_likwid_markerRegionEvents_rocm);
    lua_register(L, "likwid_markerRegionMetrics_rocm", lua_likwid_markerRegionMetrics_rocm);
    lua_register(L, "likwid_markerRegionGpus_rocm", lua_likwid_markerRegionGpus_rocm);
    lua_register(L, "likwid_markerRegionGpulist_rocm", lua_likwid_markerRegionGpulist_rocm);
    lua_register(L, "likwid_markerRegionTime_rocm", lua_likwid_markerRegionTime_rocm);
    lua_register(L, "likwid_markerRegionCount_rocm", lua_likwid_markerRegionCount_rocm);
    lua_register(L, "likwid_markerRegionResult_rocm", lua_likwid_markerRegionResult_rocm);
    lua_register(L, "likwid_markerRegionMetric_rocm", lua_likwid_markerRegionMetric_rocm);
#endif /* LIKWID_WITH_ROCMON */
