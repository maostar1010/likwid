...
#define DECLAREFUNC_NVML(funcname, funcsig)                                                        \
    nvmlReturn_t NVMLWEAK funcname funcsig;                                                        \
    nvmlReturn_t(*funcname##_ptr) funcsig;
...
DECLAREFUNC_NVML(nvmlDeviceGetTemperature,
...
DECLAREFUNC_NVML(nvmlDeviceGetPowerManagementLimitConstraints,
...
#define DECLAREFUNC_CUPTI(funcname, funcsig)                                                       \
    CUptiResult CUPTIWEAK funcname funcsig;                                                        \
    CUptiResult(*funcname##_ptr) funcsig;
...
static int _nvml_wrapper_getMaxClock(nvmlDevice_t device, NvmlEvent *event, NvmlEventResult *result)
{
    ...
    NVML_CALL(nvmlDeviceGetClock,
        (device, event->options.clock, NVML_CLOCK_ID_CUSTOMER_BOOST_MAX, &clock),
        return -1);
    _nvml_resultAddMeasurement(result, clock);
    ...
    nvmlDevice_t device, NvmlEvent *event, NvmlEventResult *result)
{
    ...
    NVML_CALL(nvmlDeviceGetDetailedEccErrors,
        (device, event->options.ecc.type, NVML_VOLATILE_ECC, &counts),
        return -1);
    ...
    nvmlDevice_t device, NvmlEvent *event, NvmlEventResult *result)
{
    ...
    NVML_CALL(nvmlDeviceGetTotalEccErrors,
        (device, event->options.ecc.type, NVML_VOLATILE_ECC, &count),
        return -1);
    ...
    nvmlDevice_t device, NvmlEvent *event, NvmlEventResult *result)
{
    ...
    NVML_CALL(
        nvmlDeviceGetPowerManagementLimitConstraints, (device, &minLimit, &maxLimit), return -1);
...
static void _nvml_resetFullValue(NvmlEventResult *result) { result->fullValue = 0; }
