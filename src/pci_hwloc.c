...
int hwloc_pci_init(uint16_t testDevice, char **socket_bus, int *nrSockets)
{
    int cntr            = 0;
    ...
    for (i = 0; i < LIKWID_HWLOC_NAME(get_nbobjs_by_type)(hwloc_topology, HWLOC_OBJ_PCI_DEVICE);
        i++) {
        ...
        if ((obj->attr->pcidev.vendor_id == testVendor) &&
            (obj->attr->pcidev.device_id == testDevice)) {
            hwloc_obj_t walk = obj->parent;
            while (walk->type != HWLOC_OBJ_SOCKET)
                walk = walk->parent;
