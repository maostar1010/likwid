...
int perfgroup_getGroups(const char *grouppath, const char *architecture, char ***groupnames,
    char ***groupshort, char ***grouplong)
{
    int i     = 0;
    size_t s  = 0;
    int fsize = 0, hsize = 0;
        ...
        while ((ep = readdir(dp))) {
            if (strncmp(&(ep->d_name[strlen(ep->d_name) - 4]), ".txt", 4) == 0) {
                ...
                if (!access(homepath, R_OK)) {
                    (*grouplong)[i] = NULL;
                    s               = sprintf(
                        (*groupnames)[i], "%.*s", (int)(strlen(ep->d_name) - 4), ep->d_name);
                    (*groupnames)[i][s] = '\0';
                    fp                  = fopen(homepath, "r");
                    while (fgets(buf, sizeof(buf), fp)) {
                        bstring bbuf = bfromcstr(buf);
                        ...
                        if (bstrncmp(bbuf, SHORT, 5) == 0) {
                            ...
                            if (linelist->qty == 1) {
                                fprintf(
                                    stderr, "Cannot read SHORT section in groupfile %s", fullpath);
                                bdestroy(bbuf);
                                bstrListDestroy(linelist);
                                continue;
                            }
...
int perfgroup_customGroup(const char *eventStr, GroupInfo *ginfo)
{
    ...
    if (cpuid_info.isIntel && i != gpu_events) {
        ...
        if ((!has_fix3) && cpuid_info.perf_num_fixed_ctr > 3 &&
            (cpuid_info.model == ICELAKE1 || cpuid_info.model == ICELAKE2 ||
                cpuid_info.model == ICELAKEX1 || cpuid_info.model == ICELAKEX2 ||
                cpuid_info.model == ROCKETLAKE || cpuid_info.model == SAPPHIRERAPIDS ||
                cpuid_info.model == EMERALDRAPIDS || cpuid_info.model == TIGERLAKE1 ||
                cpuid_info.model == TIGERLAKE2 || cpuid_info.model == GRANITERAPIDS)) {
...
int perfgroup_readGroup(
    const char *grouppath, const char *architecture, const char *groupname, GroupInfo *ginfo)
{
    FILE *fp;
    int s, err = 0;
    char buf[1024];
...
#pragma GCC diagnostic ignored "-Wnonnull"
    if (bdata(fullpath) && access(bdata(fullpath), R_OK)) {
        DEBUG_PRINT(DEBUGLEV_INFO,
            "Cannot read group file %s. Trying %s",
            bdata(fullpath),
            bdata(homepath));
        if (access(bdata(homepath), R_OK)) {
            ERROR_PRINT("Cannot read group file %s.txt. Searched in %s and %s",
                groupname,
                bdata(fullpath),
                bdata(homepath));
            bdestroy(REQUIRE);
    ...
    while (fgets(buf, 1023 * sizeof(char), fp)) {
        ...
        if (strncmp(groupFileSectionNames[GROUP_SHORT],
                buf,
                strlen(groupFileSectionNames[GROUP_SHORT])) == 0) {
            sec = GROUP_SHORT;
            size_t i;
            ...
            continue;
        } else if (strncmp(groupFileSectionNames[GROUP_EVENTSET],
                       buf,
                       strlen(groupFileSectionNames[GROUP_EVENTSET])) == 0) {
            sec = GROUP_EVENTSET;
            continue;
        } else if (strncmp(groupFileSectionNames[GROUP_METRICS],
                       buf,
                       strlen(groupFileSectionNames[GROUP_METRICS])) == 0) {
            sec = GROUP_METRICS;
            continue;
        } else if (strncmp(groupFileSectionNames[GROUP_LONG],
                       buf,
                       strlen(groupFileSectionNames[GROUP_LONG])) == 0) {
            sec = GROUP_LONG;
            continue;
        } else if (strncmp(groupFileSectionNames[GROUP_LUA],
                       buf,
                       strlen(groupFileSectionNames[GROUP_LUA])) == 0) {
            sec = GROUP_LUA;
            continue;
        }
        if (sec == GROUP_NONE)
        ...
        if (sec == GROUP_EVENTSET) {
            ...
            for (int i = 0; i < linelist->qty; i++)
                btrimws(linelist->entry[i]);
            ginfo->counters[ginfo->nevents] =
                malloc((blength(linelist->entry[0]) + 1) * sizeof(char));
            if (ginfo->counters[ginfo->nevents] == NULL) {
                err = -ENOMEM;
                goto cleanup;
            }
            ginfo->events[ginfo->nevents] =
                malloc((blength(linelist->entry[1]) + 1) * sizeof(char));
            ...
            bstrListDestroy(linelist);
            linelist = bsplit(bbuf, ' ');
            ginfo->metricformulas[ginfo->nmetrics] =
                malloc((blength(linelist->entry[linelist->qty - 1]) + 1) * sizeof(char));
            ...
            ginfo->metricnames[ginfo->nmetrics] = malloc(
                ((blength(bbuf) - blength(linelist->entry[linelist->qty - 1])) + 1) * sizeof(char));
            ...
            bdestroy(bbuf);
            sprintf(ginfo->metricformulas[ginfo->nmetrics],
                "%s",
                bdata(linelist->entry[linelist->qty - 1]));
            bbuf = bstrcpy(linelist->entry[0]);
...
int perfgroup_new(GroupInfo *ginfo)
{
    if (!ginfo)
        return -EINVAL;
    ginfo->groupname      = NULL;
...
char *perfgroup_getEventStr(GroupInfo *ginfo)
{
    ...
    size += strlen(ginfo->events[ginfo->nevents - 1]) +
            strlen(ginfo->counters[ginfo->nevents - 1]) + 1 + 1;
    size++;
    ...
    size += sprintf(&(string[size]),
        "%s:%s",
        ginfo->events[ginfo->nevents - 1],
        ginfo->counters[ginfo->nevents - 1]);
    string[size] = '\0';
    return string;
}

void perfgroup_returnEventStr(char *eventset)
{
    if (eventset != NULL) {
        free(eventset);
        eventset = NULL;
...
int perfgroup_addMetric(GroupInfo *ginfo, char *mname, char *mcalc)
{
    ...
    if (!ginfo->metricnames) {
        ERROR_PRINT("Cannot increase space for metricnames to %zu bytes",
            (ginfo->nmetrics + 1) * sizeof(char *));
        return -ENOMEM;
    }
    ginfo->metricformulas = realloc(ginfo->metricformulas, (ginfo->nmetrics + 1) * sizeof(char *));
    if (!ginfo->metricformulas) {
        ERROR_PRINT("Cannot increase space for metricformulas to %zu bytes",
            (ginfo->nmetrics + 1) * sizeof(char *));
        return -ENOMEM;
    }
    ginfo->metricnames[ginfo->nmetrics] = malloc((strlen(mname) + 1) * sizeof(char));
    if (!ginfo->metricnames[ginfo->nmetrics]) {
        ERROR_PRINT("Cannot increase space for metricname to %zu bytes",
    ...
    if (!ginfo->metricformulas[ginfo->nmetrics]) {
        ERROR_PRINT("Cannot increase space for metricformula to %zu bytes",
            (strlen(mcalc) + 1) * sizeof(char));
        return -ENOMEM;
    }
        ...
        for (int j = 0; j < grp2->nevents; j++) {
            if (strncmp(grp1->counters[i], grp2->counters[j], strlen(grp1->counters[i])) == 0) {
                if (strncmp(grp1->events[i], grp2->events[j], strlen(grp1->events[i])) != 0) {
                    DEBUG_PRINT(DEBUGLEV_INFO,
                        "Cannot merge groups because counter %s is used for different events: %s "
                        "and %s",
                        grp1->counters[i],
                        grp1->events[i],
                        grp2->events[j]);
                    ...
                    DEBUG_PRINT(DEBUGLEV_DEVELOP,
                        "Counter %s used in both groups but measure the same event %s",
                        grp1->counters[i],
                        grp1->events[i]);
                }
            }
