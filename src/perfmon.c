...
int perfmon_init_maps(void)
{
    ...
    if ((eventHash != NULL) && (err == 0)) {
        ...
        if (tmp) {
            ...
            int ret                                = snprintf(
                eventHash[perfmon_numArchEvents].limit, blength(blim) + 1, "%s", bdata(blim));
            if (ret > 0) {
...
char **getArchRegisterTypeNames() { return archRegisterTypeNames; }
...
int perfmon_readCounters(void) { return __perfmon_readCounters(-1, -1); }
...
int perfmon_readGroupCounters(int groupId) { return __perfmon_readCounters(groupId, -1); }
