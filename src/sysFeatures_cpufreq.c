    ...
    const LikwidDevice_t device, char **value, const char *sysfs_filename)
{
    ...
    bstring filename =
        bformat("/sys/devices/system/cpu/cpu%d/cpufreq/%s", device->id.simple.id, sysfs_filename);
...
static int cpufreq_sysfs_setter(
    const LikwidDevice_t device, const char *value, const char *sysfs_filename)
{
    int err = 0;
    if ((!device) || (!value) || (!sysfs_filename) || (device->type != DEVICE_TYPE_HWTHREAD)) {
        return -EINVAL;
...
static int cpufreq_driver_test(const char *testgovernor)
{
    ...
    for (unsigned i = 0; i < topo->numHWThreads; i++) {
        ...
        if (t->inCpuSet) {
            bstring filename =
                bformat("/sys/devices/system/cpu/cpu%d/cpufreq/scaling_driver", t->apicId);
            if (!access(bdata(filename), R_OK)) {
                bstring content = read_file(bdata(filename));
                btrimws(content);
...
static _SysFeature cpufreq_acpi_features[] = {
    { "cur_cpu_freq",
     ...
     cpufreq_acpi_cur_cpu_freq_getter,    NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     "kHz" },
    { "min_cpu_freq",
     "cpu_freq", "Minimal CPU frequency",
     cpufreq_acpi_min_cpu_freq_getter,    NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     "kHz" },
    { "max_cpu_freq",
     "cpu_freq", "Maximal CPU frequency",
     cpufreq_acpi_max_cpu_freq_getter,    NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     "kHz" },
    { "avail_freqs",
     "cpu_freq", "Available CPU frequencies",
     cpufreq_acpi_avail_cpu_freqs_getter, NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL  },
    { "governor",
     "cpu_freq", "CPU frequency governor",
     cpufreq_acpi_governor_getter,        /*cpufreq_acpi_governor_setter*/ NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL  },
    { "avail_governors",
     "cpu_freq", "Available CPU frequency governors",
     cpufreq_acpi_avail_governors_getter, NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL  },
};
...
static int cpufreq_intel_pstate_test(void) { return cpufreq_driver_test("intel_pstate"); }

static _SysFeature cpufreq_pstate_features[] = {
    { "base_freq",
     "cpu_freq", "Base CPU frequency",
     cpufreq_intel_pstate_base_cpu_freq_getter,   NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     "kHz" },
    { "cur_cpu_freq",
     "cpu_freq", "Current CPU frequency",
     cpufreq_intel_pstate_cur_cpu_freq_getter,    NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     "kHz" },
    { "min_cpu_freq",
     "cpu_freq", "Minimal CPU frequency",
     cpufreq_intel_pstate_min_cpu_freq_getter,    cpufreq_intel_pstate_min_cpu_freq_setter,
     DEVICE_TYPE_HWTHREAD, NULL,
     "kHz" },
    { "max_cpu_freq",
     "cpu_freq", "Maximal CPU frequency",
     cpufreq_intel_pstate_max_cpu_freq_getter,    cpufreq_intel_pstate_max_cpu_freq_setter,
     DEVICE_TYPE_HWTHREAD, NULL,
     "kHz" },
    { "governor",
     "cpu_freq", "CPU frequency governor",
     cpufreq_intel_pstate_governor_getter,        cpufreq_intel_pstate_governor_setter,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL  },
    { "avail_governors",
     "cpu_freq", "Available CPU frequency governors",
     cpufreq_intel_pstate_avail_governors_getter, NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL  },
};
...
static int cpufreq_intel_cpufreq_test(void) { return cpufreq_driver_test("intel_cpufreq"); }
...
static int cpufreq_cppc_test(void) { return cpufreq_driver_test("cppc_cpufreq"); }
...
static _SysFeature cpufreq_cppc_features[] = {
    ...
    { "max_cpu_freq",
     ...
     "kHz" },
    { "boost",
     "cpu_freq", "Turbo boost",
     cpufreq_cppc_boost_getter,                   cpufreq_cppc_boost_setter,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL  },
    { "governor",
     "cpu_freq", "CPU frequency governor",
...
static int cpufreq_apple_cpufreq_test(void) { return cpufreq_driver_test("apple-cpufreq"); }
...
static int cpufreq_epp_test(void)
{
    if ((!access("/sys/devices/system/cpu/cpu0/cpufreq/energy_performance_preference", R_OK)) &&
        (!access("/sys/devices/system/cpu/cpu0/cpufreq/energy_performance_available_preferences",
            R_OK))) {
        return 1;
    }
    return 0;
...
static _SysFeature cpufreq_epp_features[] = {
    { "epp",
     "cpu_freq", "Current energy performance preference",
     cpufreq_intel_pstate_epp_getter,        NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL },
    { "avail_epps",
     "cpu_freq", "Available energy performance preferences",
     cpufreq_intel_pstate_avail_epps_getter, NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL },
};
...
static _SysFeature cpufreq_scaling_driver_features[] = {
    { "scaling_driver",
     "cpu_freq", "Kernel Scaling Driver",
     cpufreq_scaling_driver_getter, NULL,
     DEVICE_TYPE_HWTHREAD, NULL,
     NULL },
};
