...
static void TIMER(TscCounter *cpu_c)
{
...
static void os_timer_start(TscCounter *time) { os_timer(time); }

static void os_timer_stop(TscCounter *time) { os_timer(time); }
#endif
...
static void getCpuSpeed(void)
{
    ...
    if (!(fpipe = (FILE *)popen(command2, "r"))) { // If fpipe is NULL
        perror("Problems with pipe");
        exit(1);
    }
