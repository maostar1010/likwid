...
static int readTopologyFile(const char *filename, cpu_set_t cpuSet)
{
    int err = 0;
    char structure[512];
    char field[512];
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if ((strcmp(structure, "cpuid_topology") == 0) && (strcmp(field, "numHWThreads") == 0)) {
            if (sscanf(line, "%s %s = %d", structure, field, &numHWThreads) != 3)
                continue;
        } else if ((strcmp(structure, "cpuid_topology") == 0) &&
                   (strcmp(field, "numCacheLevels") == 0)) {
    ...
    for (size_t i = 0; i < numa_info.numberOfNodes; i++) {
        numa_info.nodes[i].processors =
            calloc(numProcessorsPerNumaNode[i], sizeof(*numa_info.nodes[i].processors));
        ...
        if (strcmp(structure, "cpuid_topology") == 0) {
            ...
            if (strcmp(field, "numSockets") == 0) {
                ...
                if (cacheLevel <= 0 || cacheLevel > numCacheLevels) {
                    ERROR_PRINT(
                        "Topology file contains cacheLevels line with level out of range: %d",
                        cacheLevel);
                    err = -EINVAL;
                    goto error;
                }
                ...
                if (strcmp(value, "type") == 0) {
                    if (sscanf(line, "%s %s %d %s = %s", structure, field, &dummy, value, type) !=
                        5)
                        continue;
                    ...
                    if (sscanf(line, "%s %s %d %s = %u", structure, field, &dummy, value, &tmp) !=
                        5)
                        continue;
            ...
            if (strcmp(field, "family") == 0) {
                ...
                if (strlen(line) < val_index)
                    continue;
                snprintf(cpuid_info.architecture,
                    sizeof(cpuid_info.architecture),
                    "%s",
                    &line[val_index]);
            }
...
int likwid_cpu_online(int cpu_id)
{
    if (cpu_id < 0)
        return 0;
    int state     = 0;
    ...
    for (i = 0; i < cpuid_topology.numHWThreads; i++) {
        ...
        if (!tree_nodeExists(currentNode, hwThreadPool[i].apicId)) {
            /*
               ...
               */
            DEBUG_PRINT(DEBUGLEV_DEVELOP,
                "Adding hwthread %d at core %d on socket %d",
...
int topology_init(void)
{
    ...
    standard_init:
        ...
        DEBUG_PRINT(
            DEBUGLEV_INFO, "Reading topology information from %s", config.topologyCfgFileName);
...
CpuTopology_t get_cpuTopology(void) { return &cpuid_topology; }

CpuInfo_t get_cpuInfo(void) { return &cpuid_info; }

NumaTopology_t get_numaTopology(void) { return &numa_info; }
