...
static int intelCpuidFunc_4(CacheLevel **cachePool)
{
    ...
    *cachePool   = (CacheLevel *)malloc(maxNumLevels * sizeof(CacheLevel));
    ...
    for (size_t i = 0; i < maxNumLevels; i++) {
        ...
        pool[i].size          = pool[i].sets * pool[i].associativity * pool[i].lineSize;
        ...
        if (i < 3) {
            if ((cpuid_info.model == NEHALEM_BLOOMFIELD) ||
                ...
                (cpuid_info.model == NEHALEM_WESTMERE_M) || (cpuid_info.model == SANDYBRIDGE) ||
                (cpuid_info.model == SANDYBRIDGE_EP) || (cpuid_info.model == IVYBRIDGE) ||
                (cpuid_info.model == IVYBRIDGE_EP) || (cpuid_info.model == HASWELL) ||
                (cpuid_info.model == HASWELL_EP) || (cpuid_info.model == HASWELL_M1) ||
                (cpuid_info.model == HASWELL_M2) || (cpuid_info.model == WESTMERE_EX) ||
        ...
        if (pool[i].threads > cpuid_topology.numCoresPerSocket * cpuid_topology.numThreadsPerCore) {
            pool[i].threads = cpuid_topology.numCoresPerSocket * cpuid_topology.numThreadsPerCore;
        }
...
static void cpuid_set_osname(void)
{
    FILE *fp;
...
int cpuid_init_cpuInfo(cpu_set_t cpuSet)
{
    ...
    DEBUG_PRINT(DEBUGLEV_DEVELOP,
        "CPU-ID CpuInfo Family %d Model %d Stepping %d isIntel %d numHWThreads %d activeHWThreads "
        "%d",
        cpuid_info.family,
        cpuid_info.model,
        cpuid_info.stepping,
    ...
    if (hasBLeaf) {
        for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
            ...
            for (level = 0; level < 3; level++) {
                ...
                switch (level) {
                case 0: /* SMT thread */
                    bitField                  = extractBitField(apicId, currOffset, 0);
        ...
        case P6_FAMILY:
            ...
            for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
                ...
                /* ThreadId is extracted from th apicId using the bit width
                     ...
                     * */
                hwThreadPool[id].threadId = extractBitField(
                    hwThreadPool[id].apicId, getBitFieldWidth(maxNumLogicalProcsPerCore), 0);
                ...
                /* CoreId is extracted from th apicId using the bitWidth
                     ...
                     * */
                hwThreadPool[id].coreId    = extractBitField(hwThreadPool[id].apicId,
                    getBitFieldWidth(maxNumCores),
                    getBitFieldWidth(maxNumLogicalProcsPerCore));

                hwThreadPool[id].packageId = extractBitField(hwThreadPool[id].apicId,
        ...
        case K8_FAMILY:
            ...
            for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
                ...
                /* ThreadId is extracted from th apicId using the bit width
                     ...
                     * */
                hwThreadPool[id].threadId = extractBitField(
                    hwThreadPool[i].apicId, getBitFieldWidth(maxNumLogicalProcsPerCore), 0);
                ...
                /* CoreId is extracted from th apicId using the bitWidth
                     ...
                     * */
                hwThreadPool[id].coreId =
                    extractBitField(hwThreadPool[i].apicId, getBitFieldWidth(maxNumCores), 0);

                hwThreadPool[id].packageId = extractBitField(hwThreadPool[i].apicId,
        ...
        case K10_FAMILY:
            ...
            for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
                ...
                hwThreadPool[id].coreId   = extractBitField(hwThreadPool[i].apicId, width, 0);
                hwThreadPool[id].packageId =
                    extractBitField(hwThreadPool[i].apicId, (8 - width), width);
                DEBUG_PRINT(DEBUGLEV_DEVELOP,
                    "I[%d] ID[%d] APIC[%d] T[%d] C[%d] P [%d]",
                    i,
                    id,
...
int cpuid_init_cacheTopology(void)
{
    ...
    case K8_FAMILY:
        ...
        if ((cachePool[0].associativity * cachePool[0].lineSize) != 0) {
            cachePool[0].sets =
                cachePool[0].size / (cachePool[0].associativity * cachePool[0].lineSize);
        }
        ...
        if ((cachePool[0].associativity * cachePool[0].lineSize) != 0) {
            cachePool[1].sets =
                cachePool[1].size / (cachePool[1].associativity * cachePool[1].lineSize);
        }
        ...
        while (type) {
            ...
            if ((type == DATACACHE) || (type == UNIFIEDCACHE)) {
                ...
                cachePool[maxNumLevels].size          = cachePool[maxNumLevels].associativity *
                                               cachePool[maxNumLevels].lineSize *
                                               cachePool[maxNumLevels].sets;
