...
int parse_cpuinfo(uint32_t *count, uint32_t *family, uint32_t *variant, uint32_t *stepping,
    uint32_t *part, uint32_t *vendor)
{
    int i         = 0;
    FILE *fp      = NULL;
    uint32_t f    = 0;
...
int hwloc_init_cpuInfo(cpu_set_t cpuSet)
{
    ...
    if (!hwloc_topology) {
        ...
        LIKWID_HWLOC_NAME(topology_set_flags)(
            hwloc_topology, HWLOC_TOPOLOGY_FLAG_INCLUDE_DISALLOWED);
        LIKWID_HWLOC_NAME(topology_set_type_filter)(
            hwloc_topology, HWLOC_OBJ_PCI_DEVICE, HWLOC_TYPE_FILTER_KEEP_ALL);
#else
        LIKWID_HWLOC_NAME(topology_set_flags)(
    ...
    cpuid_topology.numHWThreads =
        LIKWID_HWLOC_NAME(get_nbobjs_by_type)(hwloc_topology, HWLOC_OBJ_PU);
#if defined(__ARM_ARCH_7A__) || defined(__ARM_ARCH_8A)
    ...
    DEBUG_PRINT(DEBUGLEV_DEVELOP,
        "HWLOC CpuInfo Family %d Model %d Stepping %d Vendor 0x%X Part 0x%X isIntel %d "
        "numHWThreads %d activeHWThreads %d",
        cpuid_info.family,
        cpuid_info.model,
        cpuid_info.stepping,
        cpuid_info.vendor,
...
int hwloc_init_nodeTopology(cpu_set_t cpuSet)
{
    ...
    if (obj) {
        maxNumCoresPerSocket = likwid_hwloc_record_objs_of_type_below_obj(
            hwloc_topology, obj, HWLOC_OBJ_CORE, NULL, NULL);
    }
    obj = LIKWID_HWLOC_NAME(get_obj_by_type)(hwloc_topology, HWLOC_OBJ_CORE, 0);
    if (obj) {
        maxNumLogicalProcsPerCore = likwid_hwloc_record_objs_of_type_below_obj(
    ...
    for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "HWLOC Thread Pool PU %d Thread %d Core %d Die %d Socket %d inCpuSet %d",
...
void hwloc_split_llc_check(CacheLevel *llc_cache)
{
    ...
    int num_threads_per_socket =
        likwid_hwloc_record_objs_of_type_below_obj(hwloc_topology, obj, HWLOC_OBJ_PU, NULL, NULL);
...
int hwloc_init_cacheTopology(void)
{
    ...
    for (d = 0; d < depth; d++) {
        ...
        if (depth_type == HWLOC_OBJ_L1CACHE || depth_type == HWLOC_OBJ_L2CACHE ||
            depth_type == HWLOC_OBJ_L3CACHE || depth_type == HWLOC_OBJ_L4CACHE ||
    ...
    for (d = depth - 1; d >= 0; d--) {
        ...
        cachePool[id].sets          = 0;
#ifdef _ARCH_PPC
        if ((cpuid_info.family == PPC_FAMILY) &&
            ((cpuid_info.model == POWER8) || (cpuid_info.model == POWER9))) {
        ...
        if ((cachePool[id].associativity * cachePool[id].lineSize) != 0) {
            cachePool[id].sets =
                cachePool[id].size / (cachePool[id].associativity * cachePool[id].lineSize);
        }
        ...
        if (obj->infos_count > 0) {
            while (!(info = LIKWID_HWLOC_NAME(obj_get_info_by_name)(obj, "inclusiveness")) &&
                   obj->next_cousin) {
...
#endif
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "HWLOC Cache Pool ID %d Level %d Size %d Threads %d",
            id,
            cachePool[id].level,
            cachePool[id].size,
...
#else
int hwloc_init_cpuInfo(void) { return; }

int hwloc_init_cpuFeatures(void) { return; }

int hwloc_init_nodeTopology(void) { return; }

int hwloc_init_cacheTopology(void) { return; }
#endif
