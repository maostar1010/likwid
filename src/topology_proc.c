...
static int get_listPosition(int ownid, bstring list)
{
...
static int fillList(int *outList, int outOffset, bstring list)
{
    int current = 0;
    ...
    for (int i = 0; i < tokens->qty; i++) {
        ...
        if (bstrchrp(tokens->entry[i], '-', 0) == BSTR_ERR) {
            ...
            if (range->qty == 2) {
                for (int j = ownatoi(bdata(range->entry[0])); j <= ownatoi(bdata(range->entry[1]));
                    j++) {
                    if (outList) {
...
#else
static int readCacheInclusiveIntel(int level) { return 0; }
static int readCacheInclusiveAMD(int level) { return 0; }
#endif
    ...
    if (NULL != (fp = fopen("/proc/cpuinfo", "r"))) {
        ...
        for (i = 0; i < tokens->qty; i++) {
            if (binstr(tokens->entry[i], 0, countString) != BSTR_ERR) {
                HWthreads++;
            } else if ((cpuid_info.model == 0) &&
                       (binstr(tokens->entry[i], 0, modelString) != BSTR_ERR)) {
#ifndef _ARCH_PPC
...
#endif
            } else if ((cpuid_info.family == 0) &&
                       (binstr(tokens->entry[i], 0, familyString) != BSTR_ERR)) {
                struct bstrList *subtokens = bsplit(tokens->entry[i], (char)':');
                bltrimws(subtokens->entry[1]);
        ...
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "PROC CpuInfo Family %d Model %d Stepping %d isIntel %d numHWThreads %d",
            cpuid_info.family,
            cpuid_info.model,
            cpuid_info.stepping,
            cpuid_info.isIntel,
    ...
    for (int i = 1; i < flaglist->qty; i++) {
        if (bisstemeqblk(flaglist->entry[i], "sse4_1", 6) == 1) {
            ...
            bcatcstr(bfeatures, "MMX ");
                   ...
                   (!(testBit(cpuid_info.featureFlags, SSE)))) {
            ...
            bcatcstr(bfeatures, "VFPV4 ");
                   ...
                   (!testBit(cpuid_info.featureFlags, VFP))) {
            ...
            bcatcstr(bfeatures, "PMULL ");
                   ...
                   (!testBit(cpuid_info.featureFlags, SVE))) {
...
int proc_init_nodeTopology(cpu_set_t cpuSet)
{
    ...
    for (uint32_t i = 0; i < cpuid_topology.numHWThreads; i++) {
        ...
        if (NULL != (fp = fopen(bdata(file), "r"))) {
            ...
            bdestroy(src);
            if (hwThreadPool[i].packageId == 0 && hwThreadPool[i].coreId == 0) {
        ...
        if (hwThreadPool[i].packageId == -1) {
            ...
            for (int j = 0; j < num_sockets * 4; j++) {
                ...
                if (!access(bdata(nodestr), F_OK)) {
                    ...
                    for (int k = 0; k < i; k++) {
                        bstring teststr =
                            bformat("/sys/devices/system/cpu/cpu%d/node%d/cpu%d", i, j, k);
...
#endif
        DEBUG_PRINT(DEBUGLEV_DEVELOP,
            "PROC Thread Pool PU %d Thread %d Core %d Die %d Socket %d inCpuSet %d",
...
void proc_split_llc_check(CacheLevel *llc_cache)
{
    ...
    int num_threads_per_socket =
        (cpuid_topology.numCoresPerSocket * cpuid_topology.numThreadsPerCore) / num_sockets;
    ...
    while ((ep = readdir(dp))) {
        if (strncmp(ep->d_name, "cpu", 3) == 0 && ep->d_name[strlen(ep->d_name) - 1] >= '0' &&
    ...
    for (int i = 0; i < nrCaches; i++) {
        ...
        if (NULL != (fp = fopen(bdata(levelStr), "r"))) {
            ...
            if ((cachePool[i].associativity * cachePool[i].lineSize) != 0) {
                cachePool[i].sets =
                    cachePool[i].size / (cachePool[i].associativity * cachePool[i].lineSize);
            }
        }
        bdestroy(levelStr);
